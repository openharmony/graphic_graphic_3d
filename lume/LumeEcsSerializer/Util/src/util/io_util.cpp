    ...
    for (auto& entry : src->GetEntries()) {
        ...
        switch (entry.type) {
            case IDirectory::Entry::Type::FILE:
                ...
                break;
            case IDirectory::Entry::Type::DIRECTORY:
                if (auto dirExists = fileManager.OpenDirectory(childDst); !dirExists) {
    ...
    if (auto dir = fileManager.OpenDirectory(folderUri)) {
        ...
        for (const auto& entry : entries) {
            if (entry.type == CORE_NS::IDirectory::Entry::FILE) {
                ...
                static BASE_NS::vector<BASE_NS::string_view> plaintextTypes {
                    ".txt",   // C++ Buld files
                    ".cpp",   // C++ source files
                    ".h",     // C++ header files
                    ".json",  // Configuration files
                    ".json5", // Harmony OS build scripts
                    ".cmake", // Build files
                    ".in",    // CMake configuration files
                    ".ets"    // Harmony OS app code
                };
    ...
    auto replace = [&text, &replaceWith](BASE_NS::string& stringContents) {
        ...
        while (pos != BASE_NS::string::npos) {
            stringContents = stringContents.replace(
                stringContents.begin() + static_cast<int64_t>(pos), stringContents.begin() +
                    static_cast<int64_t>(pos + text.size()), replaceWith);
    ...
    auto replace = [&replacements](BASE_NS::string& stringContents) {
        for (const auto& repl : replacements) {
            ...
            while (pos != BASE_NS::string::npos) {
                stringContents = stringContents.replace(
                    stringContents.begin() + static_cast<int64_t>(pos), stringContents.begin() +
		        static_cast<int64_t>(pos + repl.from.size()), repl.to);
                pos += repl.to.size();
...
void ReplaceInString(BASE_NS::string& string, const BASE_NS::string& target, const BASE_NS::string& replacement)
{
    ...
    while (pos != BASE_NS::string::npos) {
        string = string.replace(string.begin() + static_cast<int64_t>(pos), string.begin() +
            static_cast<int64_t>(pos + target.size()), replacement);
