    ...
    const IEntityCollection& ec, const json::value& jsonIn, EntityReference& entityOut)
{
    ...
    if (renderHandleJson) {
        // Try to find a named render handle by desc.
...
bool EcsSerializer::WriteEntityCollection(const IEntityCollection& ec, json::standalone_value& jsonOut) const
{
    ...
    if (collectionCount > 0) {
        ...
        for (size_t i = 0; i < collectionCount; ++i) {
            ...
            if (!collection || collection->IsMarkedDestroyed()) {
    ...
    ec.GetEntitiesRecursive(false, allEntities);
...
bool EcsSerializer::WriteComponents(const IEntityCollection& ec, Entity entity, json::standalone_value& jsonOut) const
{
    ...
    for (const auto& uidString : managerUids) {
    ...
    IComponentManager::ComponentId id, json::standalone_value& jsonOut) const
{
    ...
    if (props) {
        ...
        for (const auto& propertyPath : *props) {
...
bool EcsSerializer::ReadComponents(IEntityCollection& ec, const json::value& jsonIn) const
{
...
RENDER_NS::RenderHandleReference EcsSerializer::LoadImageResource(BASE_NS::string_view uri) const
{
    ...
    // Check if there's already a GPU image for this file and re-use if there is.
    RenderHandleReference imageHandle = gpuResourceMgr.GetImageHandle(uri);
    if (imageHandle) {
        return imageHandle;
    }
    ...
    const auto params = PathUtil::GetUriParameters(uri);
    const auto loaderFlags = params.find("loaderFlags");
    ...
    if (imageLoadResult.success) {
        const GpuImageDesc gpuDesc = gpuResourceMgr.CreateGpuImageDesc(imageLoadResult.image->GetImageDesc());
...
{
    return IEcsSerializer::Ptr { new EcsSerializer(renderContext) };
}
