...
public:
    ...
    {
        CORE_UNUSED(taskIndex);
        CORE_UNUSED(taskCount);
    }
...
private:
    void StartLoading()
    {
        ...
        // Separate different loaders and make it possible to register more.
        switch (type) {
    ...
    void GltfImportFinished()
    {
        ...
        const string entityPath = (rootParam != params.cend()) ? rootParam->second : "";
        ...
        if (importer_) {
            ...
            // a little backwards to first create everything and then delete the extra.
            ...
            // Add all resources in separate sub-collections. Not just 3D nodes.
            {
                ...
                auto& materialCollection = ec_.AddSubCollection("materials", {});
                materialCollection.AddEntities(importResult.data.materials);

                auto& meshCollection = ec_.AddSubCollection("meshes", {});
                meshCollection.AddEntities(importResult.data.meshes);

                // Save names of materials and meshes so inserting new ones do not break
                // changes done in the editor.
                auto* ncm = GetManager<INameComponentManager>(ecs_);

                for (const auto& materialEntity : importResult.data.materials) {
                    if (auto nameComponent = ncm->Read(materialEntity)) {
                        materialCollection.SetId(nameComponent->name, materialEntity);
                    }
                }

                for (const auto& meshEntity : importResult.data.meshes) {
                    if (auto nameComponent = ncm->Read(meshEntity)) {
                        meshCollection.SetId(nameComponent->name, meshEntity);
                    }
                }
