...
private:
    ...
    void RenderSingleCommandList(RenderCommandContext& renderCommandCtx, PendingDestroyFrame& pendingFrame,
        BASE_NS::vector<MlnDataGraph>& outDataGraphs, BASE_NS::vector<DataGraphResourceInfo>& outDgResources,
        void* additionalRpSegs = nullptr, void* outRpSegsOnly = nullptr);
    ...
    bool HandleStateCommand(const void* refPtr, void* statePtr, void* currentRPPtr, uint32_t currentSubpassIndex,
        NodeContextPsoManager& psoMgr, NodeContextDescriptorSetManager& descriptorSetMgr);
    ...
    void WalkSecondaryCtx(RenderCommandContext& renderCommandCtx, void* outRpSegsOnlyVoid,
    ...
    void WalkPrimaryCtx(RenderCommandContext& renderCommandCtx, PendingDestroyFrame& pendingFrame,
        BASE_NS::vector<MlnDataGraph>& outDataGraphs, BASE_NS::vector<DataGraphResourceInfo>& outDgResources,
    ...
    void BuildTransferDg(void* transferOpsVec, void* transferDstImagesVec, void* transferDstBuffersVec,
    ...
    void BuildComputeDg(const void* computeDispatchGroupPtr, PendingDestroyFrame& pendingFrame,
        BASE_NS::vector<MlnDataGraph>& outDataGraphs, BASE_NS::vector<DataGraphResourceInfo>& outDgResources);
    void BuildAccelStructDg(const RenderCommandBuildAccelerationStructure& renderCmd, PendingDestroyFrame& pendingFrame,
        BASE_NS::vector<MlnDataGraph>& outDataGraphs, BASE_NS::vector<DataGraphResourceInfo>& outDgResources);
    ...
    MlnRenderTarget BuildRenderTarget(const BASE_NS::vector<MlnAttachmentDescriptor>& colorAttachments,
        const MlnAttachmentDescriptor* depthAttachment, const MlnAttachmentDescriptor* stencilAttachment,
        uint32_t areaWidth, uint32_t areaHeight, uint32_t diagSubpassIndex, uint32_t diagBeginType,
    ...
    void BuildOGFromState(
        const void* inputsPtr, PendingDestroyFrame& pendingFrame, BASE_NS::vector<MlnObjectGroup>& outObjectGroups);
    void BuildOGFromState(const void* inputsPtr, PendingDestroyFrame& pendingFrame, MlnObjectGroup* outOG);
    ...
    void BuildOGsFromDrawGroups(const void* rpSegPtr, uint32_t activeSubpassIndex, PendingDestroyFrame& pendingFrame,
    ...
    void BuildSingleOGFromDrawGroup(const void* dgPtr, const void* rpDescPtr, PendingDestroyFrame& pendingFrame,
        BASE_NS::vector<MlnObjectGroup>& outObjectGroups);
    ...
    //   rpSegPtr: const RenderPassSegment* (anon-ns type)
    MlnObjectGroup BuildOGFromDrawCommand(const void* statePtr, const void* cmdPtr, const void* rpSegPtr,
    ...
    MlnObjectGroup BuildOGFromDrawIndirectCommand(const void* statePtr, const void* cmdPtr, const void* rpSegPtr,
        uint32_t subpassIndex, PendingDestroyFrame& pendingFrame);
    ...
    void BuildGraphicsDg(const void* beginCmdPtr, uint32_t activeSubpassIndex,
        const BASE_NS::vector<MlnObjectGroup>& objectGroups, uint32_t rpSegIdx, PendingDestroyFrame& pendingFrame,
        BASE_NS::vector<MlnDataGraph>& outDataGraphs, BASE_NS::vector<DataGraphResourceInfo>& outDgResources);
    ...
    // 3-slot design: each buffering slot maintains its own OG pool.
    // Frame N and Frame N+bufferingCount use the same slot, same MlnBindingSet
    // handles, and same ring-buffer offsets → OGs can be fully reused with zero update.
    struct OGPoolEntry {
        uint64_t lastFrameUsed{0};
        uint16_t lastModifierMask{0};
        MlnObjectGroup objectGroup{MLN_NULL_HANDLE};
        uint64_t contentFingerprint{0};
    };
    struct OGSlot {
        uint32_t frameAllocIndex{0};
        BASE_NS::vector<OGPoolEntry> entries;
        BASE_NS::vector<OGPoolEntry> pendingEntries;
    };
    struct OGPool {
        static constexpr uint32_t SLOT_COUNT = 6u;  // DeviceConstants::MAX_BUFFERING_COUNT
        OGSlot slots[SLOT_COUNT];
    };
    static constexpr uint32_t OG_CACHE_MAX_PER_SLOT = 1024;
