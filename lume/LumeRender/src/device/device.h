...
public:
    ...
    virtual void CreateDeviceSwapchain(const SwapchainCreateInfo& swapchainCreateInfo) = 0;
    virtual void DestroyDeviceSwapchain() = 0;

    // Lock/Unlock access for this frame's GPU resources (for safety reasons)
    void SetLockResourceBackendAccess(const bool value);
    // Set render backend running
    void SetRenderBackendRunning(const bool value);
    ...
    // set backbuffer (swapchain) client handle
    void SetBackbufferHandle(const RenderHandle handle);
    // get backbuffer client handle
    RenderHandle GetBackbufferHandle() const;
    ...
    virtual BASE_NS::unique_ptr<GpuAccelerationStructure> CreateGpuAccelerationStructure(
        const GpuAccelerationStructureDesc& desc) = 0;
    ...
    virtual BASE_NS::unique_ptr<GraphicsPipelineStateObject> CreateGraphicsPipelineStateObject(
        ...
        const DynamicStateFlags dynamicStateFlags, const RenderPassDesc& renderPassDesc,
...
protected:
    ...
    BASE_NS::unique_ptr<Swapchain> swapchain_;
    RenderHandle backbufferHandle_;
