...
namespace {
constexpr uint32_t INVALID_PENDING_INDEX{~0u};
constexpr uint32_t MAX_IMAGE_EXTENT{32768u};  // should be fetched from the device

// Maximum single allocation request size, 512MB
constexpr uint32_t MAX_SINGLE_ALLOC_SIZE = 512 * 1024 * 1024;

constexpr MemoryPropertyFlags NEEDED_DEVICE_MEMORY_PROPERTY_FLAGS_FOR_STAGING_MEM_OPT{
...
void ValidateGpuImageDesc(const GpuImageDesc& desc, const string_view name)
{
    ...
    if (desc.width == 0 || desc.height == 0 || desc.depth == 0) {
        PLUGIN_LOG_E("RENDER_VALIDATION: Image extents must not be zero (x:%u, y:%u, z:%u)",
            desc.width,
            desc.height,
            desc.depth);
        valid = false;
...
void ValidateGpuImageCopy(const GpuImageDesc& desc, const BufferImageCopy& copy, const string_view name)
{
    ...
    const Size3D imageSize{std::max(1u, desc.width >> mip), std::max(1u, desc.height >> mip), desc.depth};
    if ((copy.imageOffset.width >= imageSize.width) || (copy.imageOffset.height >= imageSize.height) ||
        ...
        PLUGIN_LOG_E(
            ...
            imageSize.height,
            imageSize.depth,
            copy.imageOffset.width,
            copy.imageOffset.height,
            copy.imageOffset.depth,
            name.data());
    }
}
#endif

inline constexpr GpuBufferDesc GetValidGpuBufferDesc(const GpuBufferDesc& desc)
{
...
inline constexpr void CheckAndEnableMemoryOptimizations(const uint32_t gpuResourceMgrFlags, GpuBufferDesc& desc)
{
...
inline constexpr bool GetScalingImageNeed(
...
inline constexpr bool GetScalingImageNeed(const GpuImageDesc& desc, const array_view<const BufferImageCopy>& copies)
{
    bool scale = false;
    if (desc.engineCreationFlags & EngineImageCreationFlagBits::CORE_ENGINE_IMAGE_CREATION_SCALE) {
        // we only support single image (single buffer image copy) scaling
...
BufferImageCopy ConvertCoreBufferImageCopy(
    const IImageContainer::SubImageDesc& bufferImageCopy, const uint32_t additionalBufferOffset)
{
    return BufferImageCopy{
        /** Buffer offset */
        bufferImageCopy.bufferOffset + additionalBufferOffset,
        /** Buffer row length */
...
void LogGpuResource(const RenderHandle& gpuHandle, const EngineResourceHandle engineHandle)
{
    ...
    PLUGIN_LOG_E("gpu %s > %" PRIx64 "=%" PRIx64 " generation: %u=%u",
        names[idx].data(),
        gpuHandle.id,
        engineHandle.id,
        RenderHandleUtil::GetGenerationIndexPart(gpuHandle),
        RenderHandleUtil::GetGenerationIndexPart(engineHandle));
}
#endif
}  // namespace
...
GpuResourceManager::GpuResourceManager(Device& device, const CreateInfo& createInfo)
    : device_(device),
      gpuResourceMgrFlags_(createInfo.flags),
...
GpuResourceManager::~GpuResourceManager()
{
    ...
    auto checkAndPrintValidation = [](const PerManagerStore& store, const string_view name) {
        ...
        const auto clientLock = std::shared_lock(store.clientMutex);
...
RenderHandle GpuResourceManager::GetRawHandle(const RenderHandle& handle) const
{
    if (RenderHandleUtil::IsValid(handle)) {
        const RenderHandleType handleType = RenderHandleUtil::GetHandleType(handle);
        const uint32_t arrayIndex = RenderHandleUtil::GetIndexPart(handle);
        if (handleType == RenderHandleType::GPU_BUFFER) {
            ...
            auto const clientLock = std::shared_lock(store.clientMutex);
            if (arrayIndex < static_cast<uint32_t>(store.clientHandles.size())) {
                return store.clientHandles[arrayIndex].GetHandle();
            }
        } else if (handleType == RenderHandleType::GPU_IMAGE) {
            auto& store = imageStore_;
            auto const clientLock = std::shared_lock(store.clientMutex);
...
RenderHandleReference GpuResourceManager::CreateStagingBuffer(const GpuBufferDesc& desc)
{
    ...
    return StoreAllocation(store, {ResourceDescriptor{desc}, {}, {}, RenderHandleType::GPU_BUFFER, ~0u, 0u}).handle;
}
    ...
    const string_view name, const RenderHandle& replacedHandle, const GpuBufferDesc& desc)
{
    ...
    if (validatedDesc.engineCreationFlags & CORE_ENGINE_BUFFER_CREATION_CREATE_IMMEDIATE) {
        ...
        StoreAllocationData sad = StoreAllocation(store,
            {ResourceDescriptor{validatedDesc},
                name,
                replacedHandle,
        ...
        return StoreAllocation(
            store, {ResourceDescriptor{validatedDesc}, name, replacedHandle, RenderHandleType::GPU_BUFFER, ~0u, 0u});
    }
}

RenderHandleReference GpuResourceManager::GetOrCreate(const string_view name, const GpuBufferDesc& desc)
{
    ...
    {
        const auto lock = std::shared_lock(bufferStore_.clientMutex);
    ...
    const string_view name, const GpuBufferDesc& desc, const array_view<const uint8_t> data)
{
    ...
    GpuBufferDesc validDesc = GetValidGpuBufferDesc(desc);
    CheckAndEnableMemoryOptimizations(gpuResourceMgrFlags_, validDesc);
    const bool useStagingBuffer = (validDesc.memoryPropertyFlags & CORE_MEMORY_PROPERTY_HOST_VISIBLE_BIT) !=
    ...
    {
        const auto lock = std::lock_guard(bufferStore_.clientMutex);
        StoreAllocationData sad = CreateBuffer(name, {}, validDesc);
        ...
        const uint32_t beginIndex = static_cast<uint32_t>(stagingOperations_.bufferCopies.size()) - 1U;
        ...
        PLUGIN_ASSERT(sad.allocationIndex < bufferStore_.pendingData.allocations.size());
        auto& allocRef = bufferStore_.pendingData.allocations[sad.allocationIndex];
        ...
        if (useStagingBuffer) {
            const uint32_t stagingBufferByteSize = static_cast<uint32_t>(copiedData.size_in_bytes());
            ReserveSpaceForStaging(stagingBufferByteSize);
            uint32_t& stagingBufferSize = stagingOperations_.stagingByteSizes.back();
            const uint32_t stagingBufferByteOffset = stagingOperations_.stagingByteSizes.back();
            const uint32_t stagingBufferIndex = static_cast<uint32_t>(stagingOperations_.stagingByteSizes.size() - 1);

            // offset to data copy
            stagingOperations_.bufferCopies[beginIndex].srcOffset += stagingBufferByteOffset;
            {
                const uint64_t newSize = static_cast<uint64_t>(stagingBufferSize) + stagingBufferByteSize;
                stagingBufferSize = static_cast<uint32_t>(
                    Align(newSize > UINT32_MAX ? UINT32_MAX : static_cast<uint32_t>(newSize), BUFFER_ALIGNMENT));
            }
            stagingOperations_.bufferToBuffer.push_back(StagingCopyStruct{StagingCopyStruct::DataType::DATA_TYPE_VECTOR,
                {},
                sad.handle,
                beginIndex,
                1,
    ...
    const string_view name, const BackendSpecificBufferDesc& backendSpecificData)
{
    ...
    const auto& finalDesc = (optionalResourceIndex < buffers.size() && buffers[optionalResourceIndex])
                                ...
                                : GpuBufferDesc{};
    auto handle = StoreAllocation(store,
        {ResourceDescriptor{finalDesc},
            name,
            {},
            RenderHandleType::GPU_BUFFER,
            optionalResourceIndex,
...
RenderHandleReference GpuResourceManager::GetOrCreate(const string_view name, const GpuImageDesc& desc)
{
    RenderHandleReference handle;
    {
        PerManagerStore& store = imageStore_;
    ...
    const string_view name, const GpuImageDesc& desc, IImageContainer::Ptr image)
{
    ...
    if (image) {
        ...
        if (IsGpuImage(sad.handle)) {
            ...
            if (image->GetData().size_bytes() > UINT32_MAX) {
                PLUGIN_LOG_E("Staging buffer data exceeds uint32_t limit");
                return {};
            }
            ...
            ReserveSpaceForStaging(stagingBufferByteSize);
            uint32_t& stagingBufferSize = stagingOperations_.stagingByteSizes.back();
            uint32_t stagingBufferByteOffset = stagingOperations_.stagingByteSizes.back();
            uint32_t stagingBufferIndex = static_cast<uint32_t>(stagingOperations_.stagingByteSizes.size() - 1);
            ...
            for (const auto& copyRef : copies) {
                stagingOperations_.bufferImageCopies.push_back(
                    ConvertCoreBufferImageCopy(copyRef, stagingBufferByteOffset));
            }
            ...
            if (newSize > UINT32_MAX) {
                PLUGIN_LOG_E("staging buffer size overflow");
                return sad.handle;
            }
            stagingBufferSize = Align(static_cast<uint32_t>(newSize), BUFFER_ALIGNMENT);

            const auto count = static_cast<uint32_t>(copies.size());
            ...
            stagingOperations_.bufferToImage.push_back(
                StagingCopyStruct{StagingCopyStruct::DataType::DATA_TYPE_IMAGE_CONTAINER,
                    {},
                    sad.handle,
                    beginIndex,
                    count,
    ...
    const array_view<const uint8_t> data, const array_view<const BufferImageCopy> bufferImageCopies)
{
    StoreAllocationData sad;
    {
        ...
        if (IsGpuImage(sad.handle)) {
            ...
            const auto stagingBufferByteSize = static_cast<uint32_t>(data.size_bytes());
            ReserveSpaceForStaging(stagingBufferByteSize);
            uint32_t& stagingBufferSize = stagingOperations_.stagingByteSizes.back();
            uint32_t stagingBufferByteOffset = stagingOperations_.stagingByteSizes.back();
            uint32_t stagingBufferIndex = static_cast<uint32_t>(stagingOperations_.stagingByteSizes.size() - 1);
...
#endif
                BufferImageCopy bic = copyRef;
                bic.bufferOffset += stagingBufferByteOffset;  // add additional staging offset
                stagingOperations_.bufferImageCopies.push_back(bic);
            }
            ...
            const auto count = static_cast<uint32_t>(bufferImageCopies.size());
            const uint32_t beginIndex = static_cast<uint32_t>(stagingOperations_.bufferImageCopies.size()) - count;

            vector<uint8_t> copiedData(data.cbegin().ptr(), data.cend().ptr());
            stagingOperations_.bufferToImage.push_back(StagingCopyStruct{StagingCopyStruct::DataType::DATA_TYPE_VECTOR,
                {},
                sad.handle,
                beginIndex,
                count,
    ...
    const string_view name, const GpuImageDesc& desc, const array_view<const uint8_t> data)
{
    ...
    if ((desc.format == Format::BASE_FORMAT_D16_UNORM_S8_UINT) ||
        (desc.format == Format::BASE_FORMAT_D24_UNORM_S8_UINT) ||
        (desc.format == Format::BASE_FORMAT_D32_SFLOAT_S8_UINT)) {
        imageAspectFlags =
            ImageAspectFlagBits::CORE_IMAGE_ASPECT_DEPTH_BIT | ImageAspectFlagBits::CORE_IMAGE_ASPECT_STENCIL_BIT;
    } else if ((desc.format == Format::BASE_FORMAT_D16_UNORM) ||
               (desc.format == Format::BASE_FORMAT_X8_D24_UNORM_PACK32) ||
               (desc.format == Format::BASE_FORMAT_D32_SFLOAT)) {
        imageAspectFlags = ImageAspectFlagBits::CORE_IMAGE_ASPECT_DEPTH_BIT;
    } else {
        imageAspectFlags = ImageAspectFlagBits::CORE_IMAGE_ASPECT_COLOR_BIT;
    }
    BufferImageCopy bufferImageCopy{
        0,
        desc.width,
        desc.height,
        {imageAspectFlags, 0, 0, desc.layerCount},
        {0, 0, 0},
    ...
    const string_view name, const GpuImageDesc& desc, const GpuImagePlatformData& gpuImagePlatformData)
{
    ...
    // replace immediate created if still pending (i.e. not usable on the GPU)
    const auto emplaceResourceIndex = static_cast<uint32_t>(store.pendingData.images.size());
    const uint32_t optionalResourceIndex =
        Math::min(emplaceResourceIndex, GetPendingOptionalResourceIndex(store, {}, name));
...
RenderHandleReference GpuResourceManager::GetOrCreate(const string_view name, const GpuSamplerDesc& desc)
{
    RenderHandleReference handle;
    {
        ...
        if (!handle) {
            handle =
                StoreAllocation(store, {ResourceDescriptor{desc}, name, {}, RenderHandleType::GPU_SAMPLER, ~0u, 0u})
                    .handle;
        }
    }
    return handle;
...
RenderHandleReference GpuResourceManager::Create(const string_view name, const GpuSamplerDesc& desc)
{
    ...
    return StoreAllocation(store, {ResourceDescriptor{desc}, name, {}, RenderHandleType::GPU_SAMPLER, ~0u, 0u}).handle;
}
    ...
    const RenderHandleReference& replacedHandle, const GpuSamplerDesc& desc)
{
    ...
    return StoreAllocation(store, {ResourceDescriptor{desc}, {}, rawHandle, RenderHandleType::GPU_SAMPLER, ~0u, 0u})
        .handle;
}
...
void GpuResourceManager::Destroy(PerManagerStore& store, const RenderHandle& handle)
{
    ...
    if (currGeneration != destroyHandleGeneration) {
        PLUGIN_LOG_W("RENDER_VALIDATION: destroy handle is not the current generation (destroy:%u != current:%u)",
            currGeneration,
            destroyHandleGeneration);
    }
#endif

    if (const uint32_t hasNameId = RenderHandleUtil::GetHasNamePart(handle); hasNameId != 0U) {
        // remove name if present
        if (auto const pos = std::find_if(store.nameToClientIndex.begin(),
                store.nameToClientIndex.end(),
    ...
    // old code: store.descriptions[arrayIndex] = {};
...
RenderHandleReference GpuResourceManager::GetHandleNoLock(const PerManagerStore& store, const string_view name)
{
...
RenderHandleReference GpuResourceManager::GetHandle(const PerManagerStore& store, const string_view name)
{
    if (name.empty()) {  // early out before lock
        return RenderHandleReference{};
    }
...
RenderHandle GpuResourceManager::GetRawHandle(const PerManagerStore& store, const string_view name)
{
    if (name.empty()) {  // early out before lock
        return RenderHandle{};
    }
...
bool GpuResourceManager::HasNamedResource(const PerManagerStore& store, const string_view name)
{
    if (name.empty()) {  // early out before lock
        return false;
    }
...
bool GpuResourceManager::HasBuffer(const BASE_NS::string_view name) const
{
    return HasNamedResource(bufferStore_, name);
}

bool GpuResourceManager::HasImage(const BASE_NS::string_view name) const
{
    return HasNamedResource(imageStore_, name);
}

bool GpuResourceManager::HasSampler(const BASE_NS::string_view name) const
{
    return HasNamedResource(samplerStore_, name);
}
...
uint64_t GpuResourceManager::GetBufferDeviceAddress(const RenderHandle& handle) const
{
    if (IsGpuBuffer(handle)) {
        const uint32_t arrayIndex = RenderHandleUtil::GetIndexPart(handle);
        const PerManagerStore& store = bufferStore_;
        ...
        if (arrayIndex < static_cast<uint32_t>(store.descriptions.size())) {
            const GpuBuffer* buffer = gpuBufferMgr_->Get(arrayIndex);
            if (buffer) {
...
uint64_t GpuResourceManager::GetBufferDeviceAddress(const RenderHandleReference& handle) const
{
    return GetBufferDeviceAddress(handle.GetHandle());
}
...
vector<RenderHandleReference> GpuResourceManager::GetHandles(const PerManagerStore& store)
{
    const auto lock = std::shared_lock(store.clientMutex);
...
vector<RenderHandle> GpuResourceManager::GetRawHandles(const PerManagerStore& store)
{
    const auto lock = std::shared_lock(store.clientMutex);

    vector<RenderHandle> res;
    res.reserve(store.clientHandles.size());
    for (const auto& ref : store.clientHandles) {
        if (ref) {
            res.push_back(ref.GetHandle());
        }
    }
    return res;
}

vector<RenderHandleReference> GpuResourceManager::GetBufferHandles() const
{
    return GetHandles(bufferStore_);
}

vector<RenderHandleReference> GpuResourceManager::GetImageHandles() const
{
    return GetHandles(imageStore_);
}

vector<RenderHandleReference> GpuResourceManager::GetSamplerHandles() const
{
    return GetHandles(samplerStore_);
}

vector<RenderHandle> GpuResourceManager::GetRawBufferHandles() const
{
    return GetRawHandles(bufferStore_);
}

vector<RenderHandle> GpuResourceManager::GetRawImageHandles() const
{
    return GetRawHandles(imageStore_);
}

vector<RenderHandle> GpuResourceManager::GetRawSamplerHandles() const
{
    return GetRawHandles(samplerStore_);
}

void GpuResourceManager::GetRawBufferHandles(const BufferHandleInfo& info, vector<RenderHandle>& handles) const
{
    const PerManagerStore& store = bufferStore_;
    {
        const auto lock = std::shared_lock(store.clientMutex);
        ...
        handles.clear();
        handles.resize(store.clientHandles.size());
        const bool fillWithDefault =
            (info.infoFlags & BufferHandleInfo::HandleInfoFlagBits::HANDLE_FILL_WITH_DEFAULT_BIT);
        for (size_t idx = 0; idx < store.clientHandles.size(); ++idx) {
            const auto& ref = store.clientHandles[idx];
            auto& currHandle = handles[idx];
            if (ref) {
                // validity flags
                const auto& desc = store.descriptions[idx].combinedBufDescriptor.bufferDesc;
                if (desc.usageFlags & info.usageFlags) {
                    currHandle = ref.GetHandle();
                } else {
                    currHandle = (fillWithDefault) ? defaultResources_.bufferHandle : RenderHandle{};
                }
            } else if (fillWithDefault) {
                currHandle = defaultResources_.bufferHandle;
            }
        }
    }
}

void GpuResourceManager::GetRawImageHandles(const ImageHandleInfo& info, vector<RenderHandle>& handles) const
{
    const PerManagerStore& store = imageStore_;
    {
        const auto lock = std::shared_lock(store.clientMutex);
        ...
        handles.resize(store.clientHandles.size());
        const bool fillWithDefault =
            (info.infoFlags & ImageHandleInfo::HandleInfoFlagBits::HANDLE_FILL_WITH_DEFAULT_BIT);
        for (size_t idx = 0; idx < store.clientHandles.size(); ++idx) {
            const auto& ref = store.clientHandles[idx];
            auto& currHandle = handles[idx];
            if (ref) {
                // validity flags
                const auto& desc = store.descriptions[idx].imageDescriptor;
                if (desc.usageFlags & info.usageFlags) {
                    currHandle = ref.GetHandle();
                } else {
                    currHandle = (fillWithDefault) ? defaultResources_.imageHandle : RenderHandle{};
                }
            } else if (fillWithDefault) {
                currHandle = defaultResources_.imageHandle;
            }
        }
    }
}

void GpuResourceManager::GetRawSamplerHandles(const SamplerHandleInfo& info, vector<RenderHandle>& handles) const
{
    const PerManagerStore& store = samplerStore_;
    {
        const auto lock = std::shared_lock(store.clientMutex);
        ...
        handles.resize(store.clientHandles.size());
        const bool fillWithDefault =
            (info.infoFlags & SamplerHandleInfo::HandleInfoFlagBits::HANDLE_FILL_WITH_DEFAULT_BIT);
        for (size_t idx = 0; idx < store.clientHandles.size(); ++idx) {
            const auto& ref = store.clientHandles[idx];
            auto& currHandle = handles[idx];
            if (ref) {
                ...
                currHandle = defaultResources_.samplerHandle;
            }
        }
    }
}
    ...
    if (resourceType == RenderHandleType::GPU_BUFFER) {
        ...
        if (RenderHandleUtil::IsGpuAccelerationStructure(op.handle)) {
            ...
            gpuBufferMgr_->Create<GpuAccelerationStructureDesc>(arrayIndex,
                op.descriptor.combinedBufDescriptor.bufferDesc,
                {},
                true,
            ...
            if (op.optionalResourceIndex != ~0u) {
                ...
                gpuBufferMgr_->Create<GpuAccelerationStructureDesc>(arrayIndex,
                    op.descriptor.combinedBufDescriptor.bufferDesc,
                    move(res[op.optionalResourceIndex]),
                    false,
                    op.descriptor.combinedBufDescriptor);
            } else {
                gpuBufferMgr_->Create<GpuAccelerationStructureDesc>(arrayIndex,
...
void GpuResourceManager::PerManagerStore::HandleRemoved(uint32_t arrayIndex, const OperationDescription& operation)
{
    PLUGIN_ASSERT(arrayIndex < static_cast<uint32_t>(clientHandles.size()));
    availableHandleIds.push_back(operation.handle.id);
    clientHandles[arrayIndex] = {};
    additionalData[arrayIndex] = {};
}

void GpuResourceManager::PerManagerStore::HandleAlloc(uint32_t arrayIndex, const OperationDescription& operation)
{
    PLUGIN_ASSERT(arrayIndex < static_cast<uint32_t>(gpuHandles.size()));
    ...
    const EngineResourceHandle gpuHandle = UnpackNewHandle(gpuHandles[arrayIndex], handleType, arrayIndex);
#if (RENDER_DEBUG_GPU_RESOURCE_IDS == 1)
    LogGpuResource(operation.handle, gpuHandle);
#endif
    gpuHandles[arrayIndex] = gpuHandle;
#if (RENDER_VULKAN_VALIDATION_ENABLED == 1)
    debugTagAllocations.push_back(operation.handle);
#endif
}
    ...
    uint32_t arrayIndex, const OperationDescription& operation, const bool isFrameEnd, const bool allowDestruction)
{
    ...
    availableHandleIds.push_back(operation.handle.id);
    if (RenderHandleUtil::IsShallowResource(operation.handle)) {
        PLUGIN_ASSERT(arrayIndex < static_cast<uint32_t>(clientHandles.size()));
        PLUGIN_ASSERT(arrayIndex < static_cast<uint32_t>(gpuHandles.size()));
        // shallow handles do not destroy the actual GPU resource
        // there's always the real handle which handles the destruction based on references
        clientHandles[arrayIndex] = {};
        additionalData[arrayIndex] = {};
        // shallow handle invalidated with default (the generation is not correct anyhow)
        gpuHandles[arrayIndex] = {};
        return false;
    }
    return true;
}
...
{
    std::unique_lock clientLock(store.clientMutex);
    // protect GPU memory allocations
    std::unique_lock allocationLock(allocationMutex_);
    ...
    const auto pendingRemaps = move(store.pendingData.remaps);
    ...
    for (auto const& allocation : pendingAllocations) {
        ...
        if (allocation.allocType == AllocType::REMOVED) {
            store.HandleRemoved(arrayIndex, allocation);
            continue;
        }
...
void GpuResourceManager::EndFrame()
{
    ...
    renderTimeReservedGpuBuffer_.values.clear();
    renderTimeReservedGpuBuffer_.executeValues.clear();
}
...
void GpuResourceManager::InvalidateGpuImageHandle(const RenderHandle& clientHandle)
{
    if (RenderHandleUtil::GetHandleType(clientHandle) != RenderHandleType::GPU_IMAGE) {
        return;
    }
    const uint32_t arrayIndex = RenderHandleUtil::GetIndexPart(clientHandle);
    PerManagerStore& store = imageStore_;
    auto const clientLock = std::lock_guard(store.clientMutex);
    if (arrayIndex < store.gpuHandles.size()) {
        // clearing clientHandles for this handle would break remapping.
        store.additionalData[arrayIndex] = {};
        store.gpuHandles[arrayIndex] = InvalidateWithGeneration(store.gpuHandles[arrayIndex]);
    }
}
...
void GpuResourceManager::LockFrameStagingData()
{
    ...
    // create frame staging buffers and set handles for staging
    {
        ...
        // handle render time reserved gpu buffer
        {
            ...
            if (rtrgb.neededByteSize > 0U) {
                ...
                rtrgb.neededByteSize = 0U;
                rtrgb.executeValues.swap(rtrgb.values);
                rtrgb.mappedData = nullptr;
            } else {
                // clear always the resource if no usage
                rtrgb = {};
            }
        }

        for (uint32_t size : perFrameStagingData_.stagingByteSizes) {
            perFrameStagingBuffers_.push_back(CreateStagingBuffer(GetStagingBufferDesc(size)));
            perFrameStagingData_.stagingBuffers.push_back(perFrameStagingBuffers_.back().GetHandle());
        }
        ...
        for (auto& ref : perFrameStagingData_.bufferToBuffer) {
            if ((!ref.invalidOperation) && (ref.stagingBufferByteSize > 0)) {
                ref.srcHandle = perFrameStagingBuffers_[ref.stagingBufferIndex];
            }
        }
    ...
    }
    {
        ...
        for (size_t idx = 0; idx < perFrameStagingData_.scalingImageData.scalingImages.size(); ++idx) {
            ...
            perFrameStagingScalingImages_[idx] = CreateImage({},
                {},
...
{
    return {move(store.pendingData.allocations),
        move(store.pendingData.buffers),
        move(store.pendingData.images),
...
EngineResourceHandle GpuResourceManager::GetGpuHandleNoValidation(
...
EngineResourceHandle GpuResourceManager::GetGpuHandle(const PerManagerStore& store, const RenderHandle& clientHandle)
{
    const uint32_t arrayIndex = RenderHandleUtil::GetIndexPart(clientHandle);
    if (arrayIndex < (uint32_t)store.gpuHandles.size()) {
        // NOTE: separate GPU handle generation counter might not
        // be exact when using shallow handles (i.e. counter is zero always after shallow handles)
#if (RENDER_VALIDATION_ENABLED == 1)
        if (!RenderHandleUtil::IsValid(store.gpuHandles[arrayIndex])) {
            const auto tmpName = to_string(clientHandle.id) + "GpuResourceManager::GetGpuHandleInvalid";
            PLUGIN_LOG_ONCE_E(tmpName.data(), "RENDER_VALIDATION : invalid gpu handle %" PRIx64, clientHandle.id);
        }
#endif
        return store.gpuHandles[arrayIndex];
    } else {
...
EngineResourceHandle GpuResourceManager::GetGpuHandleNoValidation(const RenderHandle& clientHandle) const
{
    const RenderHandleType handleType = RenderHandleUtil::GetHandleType(clientHandle);
    if (handleType == RenderHandleType::GPU_BUFFER) {
        return GetGpuHandleNoValidation(bufferStore_, clientHandle);
    } else if (handleType == RenderHandleType::GPU_IMAGE) {
        return GetGpuHandleNoValidation(imageStore_, clientHandle);
    } else if (handleType == RenderHandleType::GPU_SAMPLER) {
        return GetGpuHandleNoValidation(samplerStore_, clientHandle);
    } else {
        return {};
    }
}
    ...
    const uint32_t additionalInfoFlags)
{
    ...
    if (type == RenderHandleType::GPU_BUFFER) {
        ...
        infoFlags |= (rd.engineCreationFlags & CORE_ENGINE_BUFFER_CREATION_MAP_OUTSIDE_RENDERER)
                         ...
                         : 0;
        // NOTE: currently tagged as shallow resource with dynamic ring buffer (offsets)
        infoFlags |= (rd.engineCreationFlags & CORE_ENGINE_BUFFER_CREATION_DYNAMIC_RING_BUFFER)
                         ? CORE_RESOURCE_HANDLE_SHALLOW_RESOURCE
                         : 0;
    ...
    PerManagerStore& store, const StoreAllocationInfo& info)
{
    ...
    if (hasReplaceHandle) {
        ...
        // re-use the name if it was present
        ...
        if (RenderHandleUtil::IsValid(data.handle.GetHandle())) {
            ...
            data.handle = RenderHandleReference(
                CreateClientHandle(
                    info.type, info.descriptor, data.handle.GetHandle().id, hasNameId, info.addHandleFlags),
        ...
        // in some cases the handle might be invalid even though it is found
    ...
    if (data.allocationIndex == ~0u) {
        ...
        store.pendingData.allocations[data.allocationIndex] = {
            data.handle.GetHandle(), info.descriptor, info.allocType, info.optResourceIndex};
    }
...
ColorSpaceFlags GpuResourceManager::GetColorSpaceFlags() const
{
    return device_.GetColorSpaceFlags();
}

Format GpuResourceManager::GetColorSpaceFormat(const Format format, const ColorSpaceFlags colorSpaceFlags) const
{
    return device_.GetColorSpaceFormat(format, colorSpaceFlags);
}

SurfaceTransformFlags GpuResourceManager::GetSurfaceTransformFlags(const RenderHandleReference& handle) const
{
    return GetSurfaceTransformFlags(handle.GetHandle());
}

SurfaceTransformFlags GpuResourceManager::GetSurfaceTransformFlags(const RenderHandle& handle) const
{
    if (IsSwapchain(handle)) {
        return device_.GetSurfaceTransformFlags(handle);
    } else {
        return 0u;
    }
}

void GpuResourceManager::MapRenderTimeGpuBuffers()
{
    if (RenderHandleUtil::IsValid(renderTimeReservedGpuBuffer_.baseHandle)) {
        renderTimeReservedGpuBuffer_.mappedData = MapBuffer(renderTimeReservedGpuBuffer_.baseHandle);
    }
}

void GpuResourceManager::UnmapRenderTimeGpuBuffers() const
{
    if (RenderHandleUtil::IsValid(renderTimeReservedGpuBuffer_.baseHandle)) {
        UnmapBuffer(renderTimeReservedGpuBuffer_.baseHandle);
    }
}

RenderHandle GpuResourceManager::ReserveRenderTimeGpuBuffer(const uint32_t byteSize)
{
    if (renderTimeState_ != RenderTimeState::RENDER_PRE_EXECUTE) {
#if (RENDER_VALIDATION_ENABLED == 1)
        PLUGIN_LOG_ONCE_W(
            "ReserveRenderTimeGpuBuffer_RENDER", "ReserveRenderTimeGpuBuffer can only be called in PreExecuteFrame()");
#endif
        return {};
    }

    const uint32_t currOffset = renderTimeReservedGpuBuffer_.neededByteSize;
    ...
    renderTimeReservedGpuBuffer_.values.push_back({currOffset, byteSize});

    constexpr RenderHandleInfoFlags infoFlags{
        RenderHandleInfoFlagBits::CORE_RESOURCE_HANDLE_RENDER_TIME_MAPPED_GPU_BUFFER};
    return RenderHandleUtil::CreateGpuResourceHandle(RenderHandleType::GPU_BUFFER, infoFlags, currIndex, 0xffU, 0U);
}

IRenderNodeGpuResourceManager::MappedGpuBufferData GpuResourceManager::AcquireRenderTimeGpuBuffer(
    const RenderHandle handle) const
{
    IRenderNodeGpuResourceManager::MappedGpuBufferData mgbd;
    if (renderTimeState_ != RenderTimeState::RENDER_EXECUTE) {
#if (RENDER_VALIDATION_ENABLED == 1)
        PLUGIN_LOG_ONCE_W(
            "AcquireRenderTimeGpuBuffer_RENDER", "AcquireRenderTimeGpuBuffer can only be called in ExecuteFrame()");
#endif
        return mgbd;
    } else if (RenderHandleUtil::GetAdditionalData(handle) &
               RenderHandleInfoFlagBits::CORE_RESOURCE_HANDLE_RENDER_TIME_MAPPED_GPU_BUFFER) {
        ...
        if (index < static_cast<uint32_t>(renderTimeReservedGpuBuffer_.executeValues.size())) {
            ...
            mgbd.handle = renderTimeReservedGpuBuffer_.baseHandle;
        }
    }
    return mgbd;
}

void GpuResourceManager::ReserveSpaceForStaging(uint32_t byteSize)
{
    if (!stagingOperations_.stagingByteSizes.empty()) {
        const uint32_t stagingByteSize = stagingOperations_.stagingByteSizes.back();
        // Reserve if larger than max allowed
        if (static_cast<uint64_t>(stagingByteSize) + byteSize > MAX_SINGLE_ALLOC_SIZE) {
            stagingOperations_.stagingByteSizes.push_back(0);
        }
    } else {
        stagingOperations_.stagingByteSizes.push_back(0);
    }
...
void GpuResourceManager::SetState(const RenderTimeState rts)
{
    renderTimeState_ = rts;
}

void GpuResourceManager::SetDefaultResources(const GpuResourceManager::DefaultResources& defaultResources)
{
    // NOTE: no validity checks at the moment
    defaultResources_ = defaultResources;
}

uint32_t GpuResourceManager::GetBufferCount() const
{
    auto& store = bufferStore_;
    const auto lock = std::shared_lock(store.clientMutex);
    return static_cast<uint32_t>(store.clientHandles.size());
}

uint32_t GpuResourceManager::GetImageCount() const
{
    auto& store = imageStore_;
    const auto lock = std::shared_lock(store.clientMutex);
    return static_cast<uint32_t>(store.clientHandles.size());
...
uint32_t GpuResourceManager::GetSamplerCount() const
{
    auto& store = samplerStore_;
    const auto lock = std::shared_lock(store.clientMutex);
    return static_cast<uint32_t>(store.clientHandles.size());
...
#if (RENDER_VULKAN_VALIDATION_ENABLED == 1)
void GpuResourceManager::AddDebugTagsForStore(PerManagerStore& store, const RenderHandleType handleType)
{
...
#if defined(CORE_EXTENT_DEBUG_GPU_RESOURCE_MGR_NAMES)
        name += "_chandle:" + to_string(handle.id) + "_idx:" + to_string(arrayIndex) + "_fr:" + frameIdx;
#endif
        if (handleType == RenderHandleType::GPU_BUFFER) {
...
void GpuResourceManager::ProcessDebugTags()
{
    ...
    AddDebugTagsForStore(bufferStore_, RenderHandleType::GPU_BUFFER);
    AddDebugTagsForStore(imageStore_, RenderHandleType::GPU_IMAGE);
    AddDebugTagsForStore(samplerStore_, RenderHandleType::GPU_SAMPLER);
}
#endif
...
RenderNodeGpuResourceManager::~RenderNodeGpuResourceManager() = default;
...
RenderHandle RenderNodeGpuResourceManager::GetRawHandle(const RenderHandle& handle) const
{
    return gpuResourceMgr_.GetRawHandle(handle);
}
...
uint64_t RenderNodeGpuResourceManager::GetBufferDeviceAddress(const RenderHandle& handle) const
{
    return gpuResourceMgr_.GetBufferDeviceAddress(handle);
}
...
ColorSpaceFlags RenderNodeGpuResourceManager::GetColorSpaceFlags() const
{
    return gpuResourceMgr_.GetColorSpaceFlags();
}

Format RenderNodeGpuResourceManager::GetColorSpaceFormat(
    const Format format, const ColorSpaceFlags colorSpaceFlags) const
{
    return gpuResourceMgr_.GetColorSpaceFormat(format, colorSpaceFlags);
}

SurfaceTransformFlags RenderNodeGpuResourceManager::GetSurfaceTransformFlags(const RenderHandle& handle) const
{
    return gpuResourceMgr_.GetSurfaceTransformFlags(handle);
}
...
vector<RenderHandle> RenderNodeGpuResourceManager::GetBufferHandles() const
{
    return gpuResourceMgr_.GetRawBufferHandles();
}

vector<RenderHandle> RenderNodeGpuResourceManager::GetImageHandles() const
{
    return gpuResourceMgr_.GetRawImageHandles();
}

vector<RenderHandle> RenderNodeGpuResourceManager::GetSamplerHandles() const
{
    return gpuResourceMgr_.GetRawSamplerHandles();
}

void RenderNodeGpuResourceManager::GetBufferHandles(
    const IGpuResourceManager::BufferHandleInfo& info, BASE_NS::vector<RenderHandle>& handles) const
{
    gpuResourceMgr_.GetRawBufferHandles(info, handles);
}

void RenderNodeGpuResourceManager::GetImageHandles(
    const IGpuResourceManager::ImageHandleInfo& info, BASE_NS::vector<RenderHandle>& handles) const
{
    gpuResourceMgr_.GetRawImageHandles(info, handles);
}

void RenderNodeGpuResourceManager::GetSamplerHandles(
    const IGpuResourceManager::SamplerHandleInfo& info, BASE_NS::vector<RenderHandle>& handles) const
{
    gpuResourceMgr_.GetRawSamplerHandles(info, handles);
}
...
RenderHandle RenderNodeGpuResourceManager::ReserveGpuBuffer(const uint32_t byteSize)
{
    return gpuResourceMgr_.ReserveRenderTimeGpuBuffer(byteSize);
}

RenderNodeGpuResourceManager::MappedGpuBufferData RenderNodeGpuResourceManager::AcquireGpubuffer(
    const RenderHandle handle)
{
    return gpuResourceMgr_.AcquireRenderTimeGpuBuffer(handle);
}

uint32_t RenderNodeGpuResourceManager::GetBufferCount() const
{
    return gpuResourceMgr_.GetBufferCount();
}

uint32_t RenderNodeGpuResourceManager::GetImageCount() const
{
    return gpuResourceMgr_.GetImageCount();
}

uint32_t RenderNodeGpuResourceManager::GetSamplerCount() const
{
    return gpuResourceMgr_.GetSamplerCount();
}
