...
// Declare and define function pointers for the needed GL functions.
extern "C" {
...
static void DoDummy()
{
    ...
    RegisterClass(&wc);
    const HWND dummyhWnd = CreateWindowEx(0,
        wc.lpszClassName,
        wc.lpszClassName,
        WS_OVERLAPPED,
        CW_USEDEFAULT,
...
uintptr_t WGLState::CreateSurface(uintptr_t window, uintptr_t /* instance */, uint32_t /* flags */) const noexcept
{
...
#undef declareWGL
    ...
    if (backendConfig) {
        ...
        addAttribute(WGL_ALPHA_BITS_ARB, 8);
    }
...
// NOTE: intentional re-inclulde of gl_functions.h
#undef GLES_FUNCTIONS_H
...
void WGLState::SetContext(const SwapchainGLES* swapChain)
{
    if (swapChain == nullptr) {
        ...
        if (plat.surface == 0) {
            ...
            PLUGIN_LOG_ONCE_E("gl_invalid_surface" + BASE_NS::to_string(swapLoc),
