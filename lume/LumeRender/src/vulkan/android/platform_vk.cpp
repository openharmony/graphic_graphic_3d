...
static constexpr string_view DEVICE_EXTENSION_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER {
...
void GetPlatformDeviceExtensions(vector<string_view>& extensions)
{
    ...
    extensions.push_back(DEVICE_EXTENSION_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER);
}
    ...
    const unordered_map<string, uint32_t>& enabledDeviceExtensions)
{
    return PlatformDeviceExtensions { enabledDeviceExtensions.contains(
        DEVICE_EXTENSION_ANDROID_EXTERNAL_MEMORY_ANDROID_HARDWARE_BUFFER) };
}
