    ...
    for (uint32_t idx = 0; idx < attachmentCount; ++idx) {
        ...
        if (const GpuImageVk* image = gpuResourceMgr.GetImage<GpuImageVk>(handle); image) {
            ...
            if ((renderPassData.viewMask > 1u) && (plat.arrayLayers > 1u)) {
                ...
                if ((!imagePlat.layerCubemapImageViews.empty()) &&
                    (attachmentDesc.mipLevel < static_cast<uint32_t>(imagePlat.layerCubemapImageViews.size()))) {
                    imageViews[viewIndex] = imagePlat.layerCubemapImageViews[attachmentDesc.mipLevel];
                } else {
