    ...
    if (device_.HasSwapchain()) {
        ...
        for (size_t swapIdx = 0; swapIdx < backBufferConfig.swapchainData.size(); ++swapIdx) {
            ...
            if (const SwapchainVk* swapchain = static_cast<const SwapchainVk*>(device_.GetSwapchain(swapData.handle));
                ...
                pi.swapchainSemaphore = platSwapchain.swapchainImages.semaphore;
                ...
                if (pi.useSwapchain) {
                    ...
                    PLUGIN_ASSERT(pi.swapchainImageIndex < (uint32_t)platSwapchain.swapchainImages.images.size());
    ...
    NodeContextPsoManager& psoMgr, const NodeContextPoolManager& poolMgr, const StateCache& stateCache)
{
    if (stateCache.validBindings) {
        if (renderCmd.indexCount) {
            ...
            stateCache.perfCounters.drawCount++;
            stateCache.perfCounters.instanceCount += renderCmd.instanceCount;
            stateCache.perfCounters.triangleCount += (renderCmd.vertexCount * 3) * renderCmd.instanceCount;
#endif
        }
    }
}
    ...
    StateCache& stateCache)
{
    ...
    const bool validRpFbo =
        stateCache.lowLevelRenderPassData.renderPass && stateCache.lowLevelRenderPassData.framebuffer;
    // invalidate the whole command list
    if (!validRpFbo) {
        stateCache.validCommandList = false;
    }

    if ((renderCmd.beginType == RenderPassBeginType::RENDER_PASS_SUBPASS_BEGIN) && validRpFbo) {
...
void ResetDescriptorSetUpdateData(DescriptorSetUpdateDataStruct& dsud)
{
    dsud.accelIndex = 0U;
    dsud.bufferIndex = 0U;
    dsud.imageIndex = 0U;
    dsud.samplerIndex = 0U;
    dsud.writeBindIdx = 0U;

    dsud.descriptorWriteCount = 0U;
}
} // namespace

void RenderBackendVk::RenderCommand(const RenderCommandUpdateDescriptorSets& renderCmd,
    const LowLevelCommandBufferVk& cmdBuf, NodeContextPsoManager& psoMgr, const NodeContextPoolManager& poolMgr,
    StateCache& stateCache, NodeContextDescriptorSetManager& ncdsm)
{
    NodeContextDescriptorSetManagerVk& aNcdsmVk = (NodeContextDescriptorSetManagerVk&)ncdsm;

    const uint32_t upDescriptorSetCount = renderCmd.descriptorSetCount;
    // NOTE: we update only 4 sets at once
    // one should typically have maximux of 4 sets for these updates
    const uint32_t maxDescSetWriteCount =
        PipelineLayoutConstants::MAX_DESCRIPTOR_SET_COUNT * PipelineLayoutConstants::MAX_DESCRIPTOR_SET_BINDING_COUNT;
    VkWriteDescriptorSet writeDescriptorSets[maxDescSetWriteCount];
    VkDescriptorBufferInfo descriptorBufferInfos[maxDescSetWriteCount];
    VkDescriptorImageInfo descriptorImageInfos[maxDescSetWriteCount];
    VkDescriptorImageInfo descriptorSamplerInfos[maxDescSetWriteCount];
#if (RENDER_VULKAN_RT_ENABLED == 1)
    VkWriteDescriptorSetAccelerationStructureKHR
        descriptorAccelInfos[PipelineLayoutConstants::MAX_DESCRIPTOR_SET_BINDING_COUNT];
#endif
    uint32_t descBatchIdx = 0U;
    ...
    for (uint32_t descIdx = 0U; descIdx < upDescriptorSetCount; ++descIdx) {
        ...
        if (descriptorSet && descriptorSet->descriptorSet) {
            ...
            const uint32_t bindingCount = static_cast<uint32_t>(bindingResources.bindings.size());
            dsud.descriptorWriteCount += bindingCount;
                ...
                if (ref.binding.descriptorType == CORE_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE) {
                    ...
                    for (uint32_t idx = 0; idx < descriptorCount; ++idx) {
                        ...
                        if (const GpuBufferVk* resPtr = gpuResourceMgr_.GetBuffer<GpuBufferVk>(bRes.handle); resPtr) {
                            ...
                            descriptorAccelInfos[dsud.accelIndex + idx] = {
                    ...
                    writeDescriptorSets[dsud.writeBindIdx++] = {
                        VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,       // sType
                        &descriptorAccelInfos[dsud.accelIndex],       // pNext
                    ...
                    for (uint32_t idx = 0; idx < descriptorCount; ++idx) {
                        ...
                        if (const GpuBufferVk* resPtr = gpuResourceMgr_.GetBuffer<GpuBufferVk>(bRes.handle); resPtr) {
                            ...
                            descriptorBufferInfos[dsud.bufferIndex + idx] = {
                                platBuffer.buffer, // buffer
                                byteOffset,        // offset
                                bufferRange,       // range
                            };
                    ...
                    writeDescriptorSets[dsud.writeBindIdx++] = {
                        ...
                        (VkDescriptorType)ref.binding.descriptorType, // descriptorType
                        nullptr,                                      // pImageInfo
                        &descriptorBufferInfos[dsud.bufferIndex],     // pBufferInfo
                        nullptr,                                      // pTexelBufferView
                    };
            ...
            for (const auto& ref : images) {
                ...
                for (uint32_t idx = 0; idx < descriptorCount; ++idx) {
                    ...
                    if (const GpuImageVk* resPtr = gpuResourceMgr_.GetImage<GpuImageVk>(bRes.handle); resPtr) {
                        ...
                        descriptorImageInfos[dsud.imageIndex + idx] = {
                            sampler,                         // sampler
                            imageView,                       // imageView
                ...
                writeDescriptorSets[dsud.writeBindIdx++] = {
                    ...
                    &descriptorImageInfos[dsud.imageIndex], // pImageInfo
                    nullptr,                                // pBufferInfo
                    nullptr,                                // pTexelBufferView
                };
            ...
            for (const auto& ref : samplers) {
                ...
                for (uint32_t idx = 0; idx < descriptorCount; ++idx) {
                    ...
                    if (const GpuSamplerVk* resPtr = gpuResourceMgr_.GetSampler<GpuSamplerVk>(bRes.handle); resPtr) {
                        ...
                        descriptorSamplerInfos[dsud.samplerIndex + idx] = {
                ...
                writeDescriptorSets[dsud.writeBindIdx++] = {
                    ...
                    descriptorCount,                              // descriptorCount
                    (VkDescriptorType)ref.binding.descriptorType, // descriptorType
                    &descriptorSamplerInfos[dsud.samplerIndex],   // pImageInfo
                    nullptr,                                      // pBufferInfo
                    nullptr,                                      // pTexelBufferView
                };
            ...
            if ((descIdx == (upDescriptorSetCount - 1U)) ||
                (descBatchIdx == PipelineLayoutConstants::MAX_DESCRIPTOR_SET_COUNT)) {
                vkUpdateDescriptorSets(device, // device
                    dsud.descriptorWriteCount, // descriptorWriteCount
                    writeDescriptorSets,       // pDescriptorWrites
                    0,                         // descriptorCopyCount
                    nullptr);                  // pDescriptorCopies

                // reset counters after update
                ResetDescriptorSetUpdateData(dsud);
#if (RENDER_PERF_ENABLED == 1)
                stateCache.perfCounters.updateDescriptorSetCount++;
#endif
            }
        } else {
            stateCache.validBindings = false;
        }
    }
}
    ...
    const StateCache& stateCache)
{
#if (RENDER_VULKAN_RT_ENABLED == 1)
    // NOTE: missing
    // const GpuBufferVk* src = gpuResourceMgr_.GetBuffer<const GpuBufferVk>(renderCmd.srcAccelerationStructure);
    ...
    if (dst && scratchBuffer) {
        ...
        // const GpuAccelerationStructurePlatformDataVk& srcPlat = src->GetPlatformDataAccelerationStructure();
