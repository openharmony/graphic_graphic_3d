...
struct DescriptorSetFillData {
    ...
    VkDescriptorSetLayout descriptorSetLayouts[PipelineLayoutConstants::MAX_DESCRIPTOR_SET_COUNT]{
...
{
    outputSpecData.constantData.constants.append(
        inputSpecConstants.constants.cbegin(), inputSpecConstants.constants.cend());

    outputSpecData.constantData.data.append(inputSpecConstants.data.cbegin(), inputSpecConstants.data.cend());
...
VkStencilOpState ConvertStencilOpState(const GraphicsState::StencilOpState& state)
{
    return VkStencilOpState{
        (VkStencilOp)state.failOp,       // failOp
        (VkStencilOp)state.passOp,       // passOp
        (VkStencilOp)state.depthFailOp,  // depthFailOp
        (VkCompareOp)state.compareOp,    // compareOp
...
struct SpecializationStageInfo {
    uint32_t vertexDataSize{0};
    uint32_t fragmentDataSize{0};
};

SpecializationStageInfo FillSpecializationMapEntries(GraphicsPipelineStateObjectVk::SpecializationData& specData)
{
    SpecializationStageInfo info;
    for (auto const& constant : specData.constantData.constants) {
        ...
        if (constant.shaderStage & CORE_SHADER_STAGE_VERTEX_BIT) {
            specData.vs.push_back(entry);
            info.vertexDataSize = Math::max(info.vertexDataSize, constant.offset + constantSize);
        }
        if (constant.shaderStage & CORE_SHADER_STAGE_FRAGMENT_BIT) {
            specData.fs.push_back(entry);
            info.fragmentDataSize = Math::max(info.fragmentDataSize, constant.offset + constantSize);
        }
    }
    return info;
}
...
MsaaConfiguration GetMsaaConfiguration(const DeviceVk& deviceVk, const DevicePlatformDataVk& devicePlatVk,
    const LowLevelRenderPassDataVk& rpData, const RenderPassSubpassDesc& subpassDesc)
{
    MsaaConfiguration config;
    if (subpassDesc.colorAttachmentCount > 0) {
        const auto& ref = rpData.renderPassCompatibilityDesc.attachments[0];
        config.sampleCount = (ref.sampleCountFlags == 0) ? VkSampleCountFlagBits::VK_SAMPLE_COUNT_1_BIT
                                                         : (VkSampleCountFlagBits)ref.sampleCountFlags;
    }

    const bool msaaEnabled =
        (VkBool32)((config.sampleCount != VkSampleCountFlagBits::VK_SAMPLE_COUNT_1_BIT) &&
                   (config.sampleCount != VkSampleCountFlagBits::VK_SAMPLE_COUNT_FLAG_BITS_MAX_ENUM)) != 0;
    if (msaaEnabled) {
        if (devicePlatVk.enabledPhysicalDeviceFeatures.sampleRateShading) {
            config.sampleShadingEnable = VK_TRUE;
            config.minSampleShading = deviceVk.GetFeatureConfigurations().minSampleShading;
        }
    }
    return config;
}
    ...
    : GraphicsPipelineStateObject(), device_(device)
{
    ...
    // geometry and tesselsation shader features are not enabled so WITH_ADJACENCY or PATCH topologies are invalid.
    const auto topology =
        Math::min(inputAssembly.primitiveTopology, PrimitiveTopology::CORE_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN);
    const VkPipelineInputAssemblyStateCreateInfo pipelineInputAssemblyStateCreateInfo{
        VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO,  // sType
        nullptr,                                                      // pNext
        pipelineInputAssemblyStateCreateFlags,                        // flags
        (VkPrimitiveTopology)topology,                                // topology
    ...
    const VkStencilOpState frontStencilOpStateVk = ConvertStencilOpState(depthStencilState.frontStencilOpState);
    const VkStencilOpState backStencilOpStateVk = ConvertStencilOpState(depthStencilState.backStencilOpState);
    ...
    std::transform(colorBlendState.colorAttachments,
        colorBlendState.colorAttachments + colAttachmentCount,
    ...
    if (dynamicStateCount > 0) {
        std::transform(dynamicStates.data(),
            dynamicStates.data() + dynamicStateCount,
            vkDynamicStates,
            [](const DynamicStateEnum& state) { return (VkDynamicState)state; });
    ...
    constexpr uint32_t maxViewportCount{1U};
    constexpr uint32_t maxScissorCount{1U};
    ...
    const auto specStageInfo = FillSpecializationMapEntries(specializationData_);
    ...
    const VkSpecializationInfo vertexSpecializationInfo{
        ...
        specStageInfo.vertexDataSize,                          // dataSize
    ...
    const VkSpecializationInfo fragmentSpecializationInfo{
        ...
        specStageInfo.fragmentDataSize,                        // dataSize
        specializationData_.constantData.data.data(),          // pData
    };

    const bool rasterizerDiscard = rasterizationState.enableRasterizerDiscard;
    const uint32_t stageCount = rasterizerDiscard ? 1U : 2U;
    const VkPipelineShaderStageCreateInfo pipelineShaderStageCreateInfos[2U]{
        {
    ...
    GetDescriptorSetFillData(deviceVk,
        pipelineLayout,
        pipelineLayoutData,
        vkDevice,
    ...
    const MsaaConfiguration msaa =
        GetMsaaConfiguration(deviceVk, devicePlatVk, lowLevelRenderPassDataVk, renderPassSubpassDescs[subpassIndex]);
    ...
    const VkPipelineMultisampleStateCreateInfo pipelineMultisampleStateCreateInfo{
        ...
        msaa.sampleCount,                                          // rasterizationSamples
        msaa.sampleShadingEnable,                                  // sampleShadingEnable
    ...
    const VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo{
        ...
        rasterizerDiscard ? nullptr : &viewportStateCreateInfo,              // pViewportState
        &pipelineRasterizationStateCreateInfo,                               // pRasterizationState
        rasterizerDiscard ? nullptr : &pipelineMultisampleStateCreateInfo,   // pMultisampleState
        rasterizerDiscard ? nullptr : &pipelineDepthStencilStateCreateInfo,  // pDepthStencilState
        rasterizerDiscard ? nullptr : &pipelineColorBlendStateCreateInfo,    // pColorBlendState
