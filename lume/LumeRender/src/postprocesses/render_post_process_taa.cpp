...
namespace {
constexpr size_t PROPERTY_BYTE_SIZE { sizeof(RenderPostProcessTaaNode::EffectProperties) };
}

RenderPostProcessTaa::RenderPostProcessTaa()
    : properties_(
          &propertiesData, array_view(PropertyType::DataType<RenderPostProcessTaaNode::EffectProperties>::properties))
{}

CORE_NS::IPropertyHandle* RenderPostProcessTaa::GetProperties()
{
    return properties_.GetData();
}

void RenderPostProcessTaa::SetData(BASE_NS::array_view<const uint8_t> data)
{
    ...
    if (data.size_bytes() != PROPERTY_BYTE_SIZE) {
        PLUGIN_LOG_ONCE_E("RenderPostProcessTaa_Size_Mismatch",
            "RENDER_VALIDATION_ENABLED: RenderPostProcessTaa::SetData(), size missmatch (ignored)");
    }
#endif
}

BASE_NS::array_view<const uint8_t> RenderPostProcessTaa::GetData() const
{
