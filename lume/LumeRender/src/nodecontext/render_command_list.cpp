    ...
    const BASE_NS::array_view<const DescriptorSetLayoutBindingResources> bindingResources)
{
    ...
    RenderHandle* descHandleData = AllocateRenderData<RenderHandle>(allocator_, count);
    if (data && descHandleData) {
        data->descriptorSetCount = count;
        data->descriptorSetHandles = descHandleData;
        for (uint32_t idx = 0; idx < count; ++idx) {
