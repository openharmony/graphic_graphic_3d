...
/**
 * @tc.name: EditKeyframeWhilePaused
 * @tc.desc: Removing then re-adding a keyframe while the track is paused must take effect immediately
 *           (e.g. merging two keyframes, or moving a keyframe in time). The remove drops the track below
 *           two keyframes, which invalidates it -- UpdateValid Stop()s it, detaching the modifier and
 *           baking a transient value. Re-adding never re-attached, so the value stayed frozen at the baked
 *           transient until a replay/reload; re-validating must re-attach so the new keyframe applies.
 * @tc.type: FUNC
 */
UNIT_TEST_F(API_TrackAnimationTest, EditKeyframeWhilePaused, testing::ext::TestSize.Level1)
{
    auto property = ConstructProperty<int>("Prop");

    static BASE_NS::vector<float> timestamps = {0.0f, 1.0f};
    static BASE_NS::vector<int> keyframes = {0, 10};
    META_NS::TrackAnimation<int> animation(CreateInstance(META_NS::ClassId::TrackAnimation));
    animation.SetKeyframes(keyframes)
        .SetTimestamps(timestamps)
        .SetProperty(property)
        .SetDuration(TimeSpan::Milliseconds(1000));
    ...
    animation.Pause();
    animation.Seek(1.0f);  // seek to the end -> caches the end segment (currentIndex_ = last keyframe)
    EXPECT_EQ(property->GetValue(), 10);

    // Remove then re-add the key (as a merge or a move-in-time does): re-add at the same timestamp with a
    // new value, then re-seek to progress.
    const float progress = animation.GetProgress();  // 1.0
    ASSERT_TRUE(animation.RemoveKeyframe(1));
    auto newVal = ConstructProperty<int>("v", 20);
    ASSERT_NE(animation.AddKeyframe(1.0f, newVal->GetValueAny().Clone(true)), ITrackAnimation::INVALID_INDEX);

    // This pause is required because the animation state went to invalid and cannot be restored (might be paused,
    // running etc)
    animation.Pause();
    animation.Seek(progress);

    // After the edit the end value must be the new keyframe (20), not the stale cached endpoint (10).
    EXPECT_EQ(property->GetValue(), 20) << "after edit+reseek: stale cached track endpoint";

    // And playing from here (Start resumes from the paused progress) must hold the new end value.
    animation.Start();
    animation.Step(this->GetTestClock());
    EXPECT_EQ(property->GetValue(), 20) << "after play: stale cached track endpoint";
}

/**
 * @tc.name: EditKeyframeRefreshesWithoutReseek
 * @tc.desc: Editing the keyframes while the track is paused must refresh the evaluated value immediately,
 *           with no re-seek. A keyframe edit changes what the track evaluates to at the current progress
 *           but does not move progress, and a Seek to the unchanged current progress is a no-op (Move only
 *           re-evaluates when progress actually changes). Without re-evaluating on the keyframe change the
 *           value stays frozen at the pre-edit evaluation.
 * @tc.type: FUNC
 */
UNIT_TEST_F(API_TrackAnimationTest, EditKeyframeRefreshesWithoutReseek, testing::ext::TestSize.Level1)
{
    auto property = ConstructProperty<int>("Prop");
    ...
    animation.Pause();
    animation.Seek(0.5f);  // seek to the middle -> interpolated 0..10 = 5
    EXPECT_EQ(property->GetValue(), 5);

    // Add a keyframe AT the current progress. This changes what the track evaluates to at 0.5, but does
    // not move progress, so a Seek(0.5) afterwards would be a no-op. Deliberately do NOT re-seek: only an
    // edit-triggered re-evaluation can refresh the value.
    auto newVal = ConstructProperty<int>("v", 100);
    ASSERT_NE(animation.AddKeyframe(0.5f, newVal->GetValueAny().Clone(true)), ITrackAnimation::INVALID_INDEX);

    EXPECT_EQ(property->GetValue(), 100)
        << "keyframe edit at the current progress did not refresh the value without a re-seek";
}
