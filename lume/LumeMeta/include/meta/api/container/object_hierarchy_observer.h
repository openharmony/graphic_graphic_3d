...
public:
    ...
    {
        META_API_CALL_INTERFACE(Observer, SetTarget(root));
        return *this;
    }
    auto& Target(const IObject::Ptr& root, HierarchyChangeModeValue mode)
    {
        META_API_CALL_INTERFACE(Observer, SetTarget(root, mode));
        return *this;
    }
    /**
    ...
    {
        return META_API_CALL_INTERFACE(Observer, GetTarget());
    }
    /**
    ...
    {
        return META_API_CALL_INTERFACE(Observer, GetAllObserved());
    }
    template<class T>
    ...
    {
        if (auto p = META_API_CALL_INTERFACE(Observer, OnHierarchyChanged())) {
            p->AddHandler(META_NS::MakeCallback<META_NS::IOnHierarchyChanged>(BASE_NS::forward<Callback>(callback)));
        }
        return *this;
    }
};
