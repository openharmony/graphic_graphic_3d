...
public:
    ...
    {
        if (auto p = META_API_CALL_INTERFACE(KeyframeAnimation, From())) {
    ...
    {
        if (auto p = META_API_CALL_INTERFACE(KeyframeAnimation, To())) {
            if (auto pv = p->GetValue()) { // IAny::Ptr
                pv->SetValue(value);
            } else {
                p->SetValue(IAny::Ptr { new Any<Type>(value) });
...
public:
    ...
    {
        return META_API_CALL_INTERFACE(TrackAnimation, Timestamps());
    }
    ...
    {
        return META_NS::ArrayProperty<Type>(META_API_CALL_INTERFACE(TrackAnimation, Keyframes()));
    }
    ...
    {
        return META_API_CALL_INTERFACE(TrackAnimation, KeyframeHandlers());
    }
    ...
    {
        return META_API_CALL_INTERFACE(TrackAnimation, AddKeyframe(timestamp, value));
    }
    bool RemoveKeyframe(size_t index)
    {
        return META_API_CALL_INTERFACE(TrackAnimation, RemoveKeyframe(index));
    }
    void RemoveAllKeyframes()
    {
        return META_API_CALL_INTERFACE(TrackAnimation, RemoveAllKeyframes());
    }
};
