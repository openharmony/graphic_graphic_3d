 ...
 * Description: Deferred callback support
...
using EnableIfDeferredFromEvent = BASE_NS::enable_if_t<IsEvent_v<Type>,
    EnableIfCanInvokeWithArguments<Func, typename Type::FunctionType>>;
*/
/**
 * @brief Make deferred callable from callable entity (e.g. lambda) by giving event type.
 * @param func Callable entity which is invoked.
 * @param queue Task queue that the call is posted to.
 */
/*
template<typename Event, typename Func, typename = EnableIfDeferredFromEvent<Event, Func>>
auto MakeDeferred(Func func, const ITaskQueue::Ptr& queue, int = 0)
{
    using DType = DeferredCallable<Func, typename Event::CallableType>;
    return typename DType::Ptr(new DType(BASE_NS::move(func), queue));
}
*/
