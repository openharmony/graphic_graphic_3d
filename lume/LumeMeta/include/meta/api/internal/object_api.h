...
#define META_API_CALL_INTERFACE(Name, Call)              \
    [&](auto* intf) {                                    \
        if (intf) {                                      \
            return intf->Call;                           \
        }                                                \
        using type = decltype(intf->Call);               \
...
public:
    ...
    {
        META_API_CALL_INTERFACE(Metadata, AddProperty(property));
    }
    /**
    ...
    {
        META_API_CALL_INTERFACE(Metadata, RemoveProperty(property));
        return *this;
    }
    /**
    ...
    {
        if (const auto meta = META_API_CACHED_INTERFACE(Metadata)) {
            if (const auto existing = meta->GetProperty(propertyName)) {
                if (Property<PropertyValueType> typed { existing }) {
                    typed->SetValue(propertyValue);
                } else {
                    CORE_LOG_E(
                        "%s: Type mismatch on property '%s'", object_->GetClassName().data(), propertyName.data());
                }
            } else {
        ...
        if (const auto meta = META_API_CACHED_INTERFACE(Metadata)) {
            if (const auto existing = meta->GetProperty(propertyName)) {
                if (auto typed = ArrayProperty<PropertyValueType>(existing)) {
                    ...
                    CORE_LOG_E("%s: Type mismatch on array property '%s'", object_->GetClassName().data(),
                        propertyName.data());
                }
            } else {
    ...
    {
        META_API_CALL_INTERFACE(Attach, Attach(attachment, dataContext));
        return *this;
    }
    ...
    {
        META_API_CALL_INTERFACE(Attach, Detach(attachment));
        return *this;
    }
    template<class T>
    ...
    {
        return META_API_CALL_INTERFACE(Attach, GetAttachments(uids, strict));
    }
    /**
    ...
    {
        return META_API_CALL_INTERFACE(Attach, template GetAttachments<T>());
    }
...
#define META_API_INTERFACE_PROPERTY_CACHED(InterfaceCachedName, PropertyName, PropertyType)    \
    ...
    {                                                                                          \
        return META_API_CALL_INTERFACE(InterfaceCachedName, PropertyName());                   \
    }                                                                                          \
