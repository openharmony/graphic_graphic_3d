...
public:
    ...
    virtual AnyReturnValue ResetValue() = 0;
...
inline bool IsItemCompatible(
    const IArrayAny& array, const TypeId& uid, CompatibilityDirection dir = CompatibilityDirection::BOTH)
{
    for (auto&& v : array.GetItemCompatibleTypes(dir)) {
        if (uid == v) {
            return true;
        }
    }
