 ...
 * Description: Object container interface
 * Author: Mikael Kilpeläinen (copied from widget container by Jani Kattelus)
 * Create: 2022-11-22
 */
...
enum class ContainerChangeType : uint8_t {
    ADDING = 1,
    ADDED = 2,
    REMOVING = 3,
    REMOVED = 4,
...
public:
    ...
    /**
     * @brief Invoked after container changed
     */
 ...
 * @param success The callee can request cancelling the operation by setting the parameter to false.
...
/**
 * @brief The IContainerPreTransaction interface can be implemented by containers which support invoking events
...
public:
    /**
     * @brief Invoked when the container is about to change
     */
    META_EVENT(IOnChildChanging, ContainerAboutToChange)
};
