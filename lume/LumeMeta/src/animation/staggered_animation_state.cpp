...
bool StaggeredAnimationState::Initialize(AnimationStateParams&& params)
{
    ...
    if (container_ = CreateContainer(); container_) {
        ...
        containerChanged_.Subscribe(container_->OnContainerChanged(), MakeCallback<IOnChildChanged>([&](const auto& i) {
...
void StaggeredAnimationState::ChildMoved(const ChildChangedInfo& info)
{
