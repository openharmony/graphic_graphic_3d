...
bool CompositeObjectProvider::Build(const IMetadata::Ptr&)
{
    ...
    OnContainerChanged()->AddHandler(MakeCallback<IOnChildChanged>(this, &CompositeObjectProvider::OnProviderChanged));
    ...
    const IObjectProvider::Ptr& provider, DataModelIndex base, size_t count)
{
    if (provider && base.IsValid()) {
        ...
        if (index != -1) {
            Invoke<IOnDataAdded>(
                META_ACCESS_EVENT(OnDataAdded), DataModelIndex { index, base.GetDimensionPointer() }, count);
        }
    }
}
    ...
    const IObjectProvider::Ptr& provider, DataModelIndex base, size_t count)
{
    if (provider && base.IsValid()) {
        size_t index = CalculateIndex(provider, base.Index());
        if (index != -1) {
            Invoke<IOnDataRemoved>(
                META_ACCESS_EVENT(OnDataRemoved), DataModelIndex { index, base.GetDimensionPointer() }, count);
        }
    }
}
    ...
    const IObjectProvider::Ptr& provider, DataModelIndex from, size_t count, DataModelIndex to)
{
    if (provider && from.IsValid() && to.IsValid()) {
        ...
        if (fromIndex != -1 && toIndex != -1) {
            Invoke<IOnDataMoved>(META_ACCESS_EVENT(OnDataMoved),
                DataModelIndex { fromIndex, from.GetDimensionPointer() }, count,
                DataModelIndex { toIndex, to.GetDimensionPointer() });
        }
    }
}

void CompositeObjectProvider::OnProviderChanged(const ChildChangedInfo& info)
{
    if (info.type == ContainerChangeType::ADDED) {
        OnProviderAdded(info);
    } else if (info.type == ContainerChangeType::REMOVED) {
        OnProviderRemoved(info);
    } else if (info.type == ContainerChangeType::MOVED) {
        OnProviderMoved(info);
    }
}
...
void CompositeObjectProvider::OnProviderAdded(const ChildChangedInfo& info)
{
    ...
    assert(!interface_cast<IContainer>(info.object));
    ...
    if (provider->GetObjectCount() > 0) {
        ...
        if (index != -1) {
            Invoke<IOnDataAdded>(META_ACCESS_EVENT(OnDataAdded), DataModelIndex { index }, provider->GetObjectCount());
        }
    }
}
...
void CompositeObjectProvider::OnProviderRemoved(const ChildChangedInfo& info)
{
    ...
    if (provider->GetObjectCount() > 0) {
        size_t index = CalculateIndexBase(info.from);
        Invoke<IOnDataRemoved>(META_ACCESS_EVENT(OnDataRemoved), DataModelIndex { index }, provider->GetObjectCount());
    }
}

void CompositeObjectProvider::OnProviderMoved(const ChildChangedInfo& info)
{
    auto provider = interface_pointer_cast<IObjectProvider>(info.object);
    ...
    if (to != -1) {
        ...
        Invoke<IOnDataMoved>(
            META_ACCESS_EVENT(OnDataMoved), DataModelIndex { from }, provider->GetObjectCount(), DataModelIndex { to });
    }
}
