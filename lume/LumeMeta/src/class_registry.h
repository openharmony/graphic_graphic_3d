 ...
 * Description: Class registry implementation
...
public:
    ...
    BASE_NS::shared_ptr<IEvent> EventOnClassRegistered(META_NS::MetadataQuery q) const override
    {
        std::unique_lock lock(mutex_);
        if (!onRegistered_ && q == MetadataQuery::CONSTRUCT_ON_REQUEST) {
            onRegistered_ = CreateShared<EventImpl<IOnClassRegistrationChanged>>("OnClassRegistered");
        }
        return onRegistered_;
    }
    BASE_NS::shared_ptr<IEvent> EventOnClassUnregistered(META_NS::MetadataQuery q) const override
    {
        std::unique_lock lock(mutex_);
        if (!onUnregistered_ && q == MetadataQuery::CONSTRUCT_ON_REQUEST) {
            onUnregistered_ = CreateShared<EventImpl<IOnClassRegistrationChanged>>("OnClassUnregistered");
        }
