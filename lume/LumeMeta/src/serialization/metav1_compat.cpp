...
public:
    ...
    ISerNode::Ptr ConstructBind(ISerNode::Ptr n)
    {
    ...
    ISerNode::Ptr RewriteBind(ISerNode::Ptr n)
    {
        if (auto obj = interface_cast<IObjectNode>(n)) {
            if (auto map = interface_cast<IMapNode>(obj->GetMembers())) {
                if (auto node = map->FindNode("Property")) {
                    return ConstructBind(node);
                }
            }
        }
        return nullptr;
    ...
    ISerNode::Ptr RewriteProperty(BASE_NS::string name, const IObjectNode& n)
    {
        ...
        if (auto m = interface_cast<IMapNode>(n.GetMembers())) {
            ...
            if (!value) {
                ...
                mapNode->AddNode("defaultValue", value);
            }
            if (bind) {
                if (auto v = mapNode->FindNode("values")) {
                    if (auto arr = interface_cast<IArrayNode>(v)) {
                        arr->AddNode(bind);
                    }
                }
            }
    ...
    void RewritePropertyMap(const NamedNode& node, BASE_NS::vector<ISerNode::Ptr>& properties)
    {
        auto n = VisitNode(node.node);
        if (auto map = interface_cast<IMapNode>(n)) {
            if (auto m = map->FindNode("__attachments")) {
                if (auto arr = interface_cast<IArrayNode>(m)) {
                    for (auto&& an : arr->GetMembers()) {
    ...
    {
        if (auto p = interface_cast<IArrayNode>(map.FindNode("__attachments"))) {
            for (auto&& n : properties) {
                p->AddNode(n);
            }
        } else {
            map.AddNode("__attachments", IArrayNode::Ptr(new ArrayNode(properties)));
        }
    }
    void Visit(const IMapNode& n) override
    {
        ...
        for (auto&& m : n.GetMembers()) {
            ...
            if (auto obj = interface_cast<IObjectNode>(m.node)) {
                ...
                RewritePropertyMap(m, properties);
                p = nullptr;
            } else {
                p = VisitNode(m.node);
            }
...
ISerNode::Ptr MetaMigrateV1::Process(ISerNode::Ptr tree)
{
    NodeVisitor v;
    tree->Apply(v);
    return v.node;
