...
void recursivelyCollectAllFiles(const std::string& aPath, std::vector<std::string>& aFiles)
{
    ...
    for (const auto& directoryName : directories)
    {
        ...
        {
            recursivelyCollectAllFiles(aPath + '/' + directoryName, aFiles);
        }
    }
}
...
void FileMonitor::scanModifications(std::vector<std::string>& aAdded, std::vector<std::string>& aRemoved, std::vector<std::string>& aModified)
{
