...
/**
 * @tc.name: ArrayPropertyEventHandlerTest
 * @tc.desc: test Diff function
 * @tc.type: FUNC
 * @tc.require: I7DMS1
 */
HWTEST_F(ArrayPropertyEventHandlerTest, Diff, TestSize.Level1)
{
    ...
    p->SetValue(BASE_NS::vector<int>{ 1, 2 });
    ExpectChanges(change, { { 2 } });

    p->SetValue(BASE_NS::vector<int>{ 1, 2, 3, 4 });
    ExpectChanges(change, { {}, { { 3, 2 }, { 4, 3 } } });

    p->SetValue(BASE_NS::vector<int>{ 1, 5, 6, 4 });
    ExpectChanges(change, { { 1, 2 }, { { 5, 1 }, { 6, 2 } } });

    p->SetValue(BASE_NS::vector<int>{ 1, 4, 5, 6 });
    ExpectChanges(change, { {}, {},  { { 3, 1 }, { 1, 2 }, { 2, 3 } } });

    p->SetValue(BASE_NS::vector<int>{ 0, 1, 6, 5 });
    ExpectChanges(change, { { 1 }, { { 0, 0 } }, { { 0, 1 }, { 3, 2 }, { 2, 3 } } });

    p->SetValue(BASE_NS::vector<int>{ 0, 1, 0 });
    p->SetValue(BASE_NS::vector<int>{ 1, 0 });
    ExpectChanges(change, { { 2 }, {},  { { 1, 0 }, { 0, 1 } } });

    p->SetValue(BASE_NS::vector<int>{ 0, 1, 0 });
    p->SetValue(BASE_NS::vector<int>{ 2, 1, 0 });
    ExpectChanges(change, { { 2 }, { { 2, 0 } }, { { 0, 2 } } });

    p->SetValue(BASE_NS::vector<int>{ 0 });
    p->SetValue(BASE_NS::vector<int> { 0, 0 });
    ExpectChanges(change, { {}, { { 0, 1 } } });

    p->SetValue(BASE_NS::vector<int>{ 0, 1, 2, 2, 1, 0, 1, 0, 1, 0 });
    p->SetValue(BASE_NS::vector<int>{ 0, 0, 2, 1, 0, 0, 1, 0, 1, 0 });
    ExpectChanges(
