...
class GraphicsContextTest : public testing::Test {
public:
 ...
 * @tc.desc: test GetInterface
 * @tc.type: FUNC
 */
HWTEST_F(GraphicsContextTest, GetInterfaceTest, TestSize.Level1)
{
    auto graphicsContext = g_context.sceneInit_->GetEngineInstance().graphicsContext_;

    ASSERT_TRUE(graphicsContext->GetInterface(IGraphicsContext::UID));
    ASSERT_TRUE(graphicsContext->GetInterface(IInterface::UID));
    ASSERT_TRUE(graphicsContext->GetInterface(IClassRegister::UID));
    ASSERT_TRUE(graphicsContext->GetInterface(IClassFactory::UID));
    ASSERT_FALSE(graphicsContext->GetInterface(IRenderContext::UID));

    const IGraphicsContext* graphicsContextConst = static_cast<const IGraphicsContext*>(graphicsContext.get());
    ASSERT_TRUE(graphicsContextConst->GetInterface(IGraphicsContext::UID));
    ASSERT_TRUE(graphicsContextConst->GetInterface(IInterface::UID));
    ASSERT_TRUE(graphicsContextConst->GetInterface(IClassRegister::UID));
    ASSERT_TRUE(graphicsContextConst->GetInterface(IClassFactory::UID));
    ASSERT_FALSE(graphicsContextConst->GetInterface(IRenderContext::UID));
}
