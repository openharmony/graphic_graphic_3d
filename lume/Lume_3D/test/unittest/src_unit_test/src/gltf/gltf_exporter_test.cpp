...
/**
 * @tc.name: ExportMeshWithExtraSubmeshDoesNotOverread
 * @tc.desc: Export pairs each MeshComponent submesh with the corresponding primitive of the source glTF. When a
 *           mesh has more submeshes than the source has primitives, pairing must not read past the primitive
 *           list. Export must not over-read (ASan) and must still complete.
 * @tc.type: FUNC
 */
UNIT_TEST(SRC_GLTFExporterTest, ExportMeshWithExtraSubmeshDoesNotOverread, testing::ext::TestSize.Level1)
{
    UTest::TestContext* testContext = UTest::GetTestContext();
    auto engine = testContext->engine;
    auto renderContext = testContext->renderContext;
    ...
    // Import a real glTF so the imported meshes carry UriComponents pointing back at their source primitives.
    constexpr string_view filename = "test://gltf/Custom/Custom.gltf";
    Entity root;
    GLTFImportResult originalGltf = LoadAndImport(filename, *gltf2, *ecs, root);
    ASSERT_TRUE(originalGltf.success);

    // Give every uri-bearing mesh more submeshes than its source has primitives. Modifying all of them
    // ensures the exported mesh overflows, and the large surplus pushes the over-read past the allocation.
    auto meshManager = GetManager<IMeshComponentManager>(*ecs);
    auto uriManager = GetManager<IUriComponentManager>(*ecs);
    ASSERT_NE(nullptr, meshManager);
    ASSERT_NE(nullptr, uriManager);
    constexpr size_t extraSubmeshes = 64u;
    size_t modifiedMeshes = 0u;
    for (IComponentManager::ComponentId i = 0; i < meshManager->GetComponentCount(); ++i) {
        const Entity entity = meshManager->GetEntity(i);
        if (!uriManager->HasComponent(entity)) {
            continue;
        }
        if (auto meshHandle = meshManager->Write(entity); meshHandle && !meshHandle->submeshes.empty()) {
            const MeshComponent::Submesh submesh = meshHandle->submeshes[0];
            for (size_t s = 0; s < extraSubmeshes; ++s) {
                meshHandle->submeshes.push_back(submesh);
            }
            ++modifiedMeshes;
        }
    }
    ASSERT_GT(modifiedMeshes, 0u);

    // The export result is not asserted — the point is that ASan must not report an out-of-bounds access.
    constexpr string_view exportFilename = "cache://ExtraSubmesh.gltf";
    const bool saved = gltf2->SaveGLTF(*ecs, exportFilename);
    (void)saved;
