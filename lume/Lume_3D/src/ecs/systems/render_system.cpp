        ...
        auto reCreateGpuImage = [](IGpuResourceManager& gpuResourceMgr, uint64_t id, RenderHandle handle,
            ...
            if (RenderHandleUtil::IsValid(handle)) {
                ...
                return gpuResourceMgr.Create(
                    DefaultMaterialCameraConstants::CAMERA_DEPTH_PREFIX_NAME + to_hex(id), desc);
            } else {
                return gpuResourceMgr.Create(
                    DefaultMaterialCameraConstants::CAMERA_COLOR_PREFIX_NAME + to_hex(id), desc);
            }
        };
...
void RenderSystem::ProcessReflections(const RenderScene& renderScene)
{
    ...
    const size_t cameraCount = dsCamera_->GetCameraCount(); // increases at every iteration
    if (queryResults.empty() || (renderScene.cameraIndex >= cameraCount)) {
        return;
    }
