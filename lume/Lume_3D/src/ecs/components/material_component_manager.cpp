...
constexpr string_view MATERIAL_COMPONENT = CORE_NS::GetName<MaterialComponent>();
...
const json::value* FindMaterialComponentJson(const json::value& metaJson)
{
    if (auto material = FindIf(metaJson.array_,
            [](const json::value& value) {
                auto nameJson = value.find(NAME);
                return nameJson && nameJson->is_string() && nameJson->string_ == MATERIAL_COMPONENT;
            });
...
void AppendProperties(vector<Property>& newProperties, vector<string>& newStringPropStorage,
    const json::value& material, const array_view<const Property> componentMetaData)
{
    if (const json::value* propertiesJson = material.find(PROPERTIES); propertiesJson && propertiesJson->is_object()) {
        for (const auto& propertyObject : propertiesJson->object_) {
            if (const auto propertyIt =
                ...
                if (propertyObject.value.is_array() && propertyIt->type.isArray &&
                    ...
                    for (const json::value& value : propertyObject.value.array_) {
    ...
    array_view<const Property> newProperties, array_view<const Property> oldProperties)
{
    ...
    for (const auto& newProp : newProperties) {
        if (newProp.type == TEXTURE_INFO_T) {
...
{
    for (const json::value& propertyValue : customProperties.array_) {
                ...
                if (object.key == "data" && object.value.is_array()) {
                    ...
                    for (const auto& dataValue : object.value.array_) {
                        if (dataValue.is_object()) {
                            ...
                            const PropertyTypeDecl typeDecl = CustomPropertyPodHelper::GetPropertyTypeDeclaration(type);
...
#include <3d/ecs/components/material_component.h>
    ...
    public:
        ...
        mutable unique_ptr<CustomPropertyPodContainer> custom_;
    };
    ...
    const MaterialComponentManager::ComponentHandle& other)
{
    if (this != &other) {
        ...
        const json::value* propertiesJson = nullptr;
