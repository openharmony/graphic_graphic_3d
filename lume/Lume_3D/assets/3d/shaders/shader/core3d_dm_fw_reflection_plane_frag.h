...
#ifndef CORE3D_DM_FW_REFLECTION_PLANE_FRAG_H
#define CORE3D_DM_FW_REFLECTION_PLANE_FRAG_H
...
vec3 GetReflectionSample(const vec2 inputUv, const float lod)
{
    ...
    BindlessImageAndSamplerIndex idx = GetImageAndSamplerIndex(0, CORE_MATERIAL_TEX_CLEARCOAT_ROUGHNESS_IDX + 1);
    return textureLod(
        sampler2D(uImages[nonuniformEXT(idx.imageIdx)], uSamplers[nonuniformEXT(idx.samplerIdx)]), inputUv.xy, lod)
        .xyz;
#else
...
vec4 PlaneReflector(const vec2 fragUv)
{
    ...
    if ((CORE_MATERIAL_FLAGS & CORE_MATERIAL_TRANSMISSION_BIT) == CORE_MATERIAL_TRANSMISSION_BIT) {
        transmission = GetTransmissionSample(inUv, 0) * GetUnpackTransmission(instanceIdx);
