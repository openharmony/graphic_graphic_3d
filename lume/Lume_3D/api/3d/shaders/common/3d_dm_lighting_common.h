...
InputBrdfData CalcBRDFMetallicRoughness(vec4 baseColor, vec3 polygonNormal, vec4 material)
{
    InputBrdfData bd;
    {
        const CORE_RELAXEDP float metallic = clamp(material.b, 0.0, 1.0);
        bd.f0.xyz =
            mix(vec3(material.a), baseColor.rgb, metallic) * baseColor.a; // f0 reflectance multiplied for transparents
        bd.f0.w = 1.0;                                                    // f90
...
InputBrdfData CalcBRDFSpecularGlossiness(vec4 baseColor, vec3 polygonNormal, vec4 material)
{
    InputBrdfData bd;
    {
...
InputBrdfData CalcBRDFSpecular(vec4 baseColor, vec3 polygonNormal, vec4 material, vec4 specular)
{
    InputBrdfData bd;
    {
        const CORE_RELAXEDP float metallic = clamp(material.b, 0.0, 1.0);
    ...
    ClearcoatShadingVariables ccsv, SheenShadingVariables ssv)
{
    ...
    float extAttenuation = 1.0;
    vec3 calculatedColor = vec3(0.0);
    if ((sd.materialFlags & CORE_MATERIAL_SHEEN_BIT) == CORE_MATERIAL_SHEEN_BIT) {
    ...
    if ((sd.materialFlags & CORE_MATERIAL_CLEARCOAT_BIT) == CORE_MATERIAL_CLEARCOAT_BIT) {
...
vec3 CalculateLighting(ShadingData sd)
{
    ...
    for (uint lightIdx = 0; lightIdx < directionalLightCount; ++lightIdx) {
        ...
        if ((sd.materialFlags & CORE_MATERIAL_SHADOW_RECEIVER_BIT) == CORE_MATERIAL_SHADOW_RECEIVER_BIT) {
        ...
        color += CalculateLight(currLightIdx, materialDiffuseBRDF, L, NoL, sd) * shadowCoeff;
    }
    ...
    if ((CORE_LIGHTING_FLAGS & CORE_LIGHTING_SPOT_ENABLED_BIT) == CORE_LIGHTING_SPOT_ENABLED_BIT) {
        ...
        for (uint spotIdx = 0; spotIdx < spotLightCount; ++spotIdx) {
            ...
            color += CalculateLight(currLightIdx, materialDiffuseBRDF, L, NoL, sd) *
    ...
    if ((CORE_LIGHTING_FLAGS & CORE_LIGHTING_POINT_ENABLED_BIT) == CORE_LIGHTING_POINT_ENABLED_BIT) {
        ...
        for (uint pointIdx = 0; pointIdx < pointLightCount; ++pointIdx) {
            ...
            const float attenuation = max(min(1.0 - pow(dist / range, 4.0), 1.0), 0.0) / (dist * dist);
            // NOTE: could check for NoL > 0.0 and NoV > 0.0
            color += CalculateLight(currLightIdx, materialDiffuseBRDF, L, NoL, sd) * attenuation;
        }
    }
