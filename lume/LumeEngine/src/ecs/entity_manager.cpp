...
private:
    // Ref count starts at -1. The internal Ptr in EntityManager bumps it to 0.
    // External EntityReferences bump it above 0. GetRefCount() == 0 means
    // no external references remain. Unref() deletes when count returns to -1.
    int32_t refcnt_{-1};
};
