...
#define DECL_NAMED_PROPERTY_METADATA(structName, prop_name, name, displayname, flags, metadata)     \
    {                                                                                               \
        ...
        (sizeof(structName::name) / sizeof(BASE_NS::remove_extent_t <decltype(structName::name)>)), \
...
template<typename T>
inline constexpr CORE_NS::ContainerApi data {
    size<T>, resize<T>, erase<T>, insert<T>, get<T>, containerElementProperty<T>
};
// clang-format on
} // namespace PropertyType
