...
template<typename ComponentType, typename BaseClass>
bool BaseManager<ComponentType, BaseClass>::Clone(IPropertyHandle& dst, const IPropertyHandle& src) const
{
    if (&src == &dst) {
        return true;
    }
    ...
    if (wdata) {
        const ComponentType* rdata = (const ComponentType*)src.RLock();
        if (rdata) {
            ::new (wdata) ComponentType(*rdata);
            result = true;
        }
        src.RUnlock();
    }
...
template<typename ComponentType, typename BaseClass>
void BaseManager<ComponentType, BaseClass>::Destroy(IPropertyHandle& dataHandle) const
{
    if (dataHandle.Owner() == this) {
        // call destructor.
        auto dstPtr = (ComponentType*)dataHandle.WLock();
        if (dstPtr) {
...
BaseManager<ComponentType, BaseClass>::BaseComponentHandle::BaseComponentHandle(BaseComponentHandle&& other) noexcept
    : rLocked_(BASE_NS::exchange(other.rLocked_, 0)), wLocked_(BASE_NS::exchange(other.wLocked_, false)),
      manager_(other.manager_), generation_(BASE_NS::exchange(other.generation_, 0)),
...
BaseManager<ComponentType, BaseClass>::BaseComponentHandle::operator=(BaseComponentHandle&& other) noexcept
{
    if (this != &other) {
        ...
        rLocked_ = BASE_NS::exchange(other.rLocked_, 0);
        wLocked_ = BASE_NS::exchange(other.wLocked_, false);
        generation_ = BASE_NS::exchange(other.generation_, 0);
