...
IFile::Ptr OhosFilesystem::OpenFile(const BASE_NS::string_view path)
{
    if (auto const pos = ohosFiles_.find(path); pos != ohosFiles_.end()) {
        ...
        if (storage) {
            CORE_LOG_I("The file is already open.");
    ...
    if (!path.empty()) {
        ...
        CORE_LOG_I("The file is opening. OhosFile count is %lu", storage->Size());
    ...
    const BASE_NS::string_view moduleName) : hapInfo_({ hapPath, bundleName, moduleName })
{
    ...
    CORE_LOG_I("Registered ohosFilesystem : 'hapPath:%s bundleName:%s moduleName:%s'",
        hapInfo_.hapPath.c_str(), hapInfo_.bundleName.c_str(), hapInfo_.moduleName.c_str());
}
...
{
    return ohosFiles_.erase(path) != ohosFiles_.end();
}
