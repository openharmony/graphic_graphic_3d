...
struct value_t {
    ...
    value_t(const value_t<OtherT>& other) : type(other.type)
    {
        switch (type) {
            case type::uninitialized:
                break;
            case type::object:
                new (&object_) object(BASE_NS::default_allocator());
                object_.reserve(other.object_.size());
                for (const auto& p : other.object_) {
                    object_.push_back({ string(p.key), p.value });
                }
                break;
            case type::array:
                new (&array_) array(BASE_NS::default_allocator());
                array_.reserve(other.array_.size());
                for (const auto& v : other.array_) {
                    array_.emplace_back(v);
                }
                break;
            case type::string:
                new (&string_) string;
    ...
    value_t(value_t&& rhs) : type { BASE_NS::exchange(rhs.type, type::uninitialized) }
    {
        switch (type) {
            case type::uninitialized:
                break;
...
const char* parse_number(const char* data, value_t<T>& res)
{
    ...
    if (data != beg) {
        ...
        if (fraction || exponent) {
            ...
            res = value(strtoll(beg, &end, 10));
        } else {
            res = value(strtoull(beg, &end, 10));
        }
        return data;
    }
    // invalid json
    ...
    while (*data) {
        ...
        if (*data == '{') {
            ...
            if (*data == ']') {
                ...
                stack.emplace_back(typename jsonValue::array {});
                acceptValue = true;
            }
        } else if (*data == ']') {
            // end of an array
...
void append(BASE_NS::string& out, const double floatingPoint)
{
    ...
    // use snprintf to calculate the required size (not supported by the _s variant)
    ...
    int ret = snprintf_s(out.data() + oldSize, newSize + 1 - oldSize, size, FLOATING_FORMAT_STR, floatingPoint);
    if (ret < 0) {
        return;
    }
}
                        ...
                        if ((i + 6U) < str.size()) {
                            ...
                            if (next[0] == '\\' && next[1] == 'u') {
                                ...
                                if (low >= 0xdc00 && low <= 0xdfff) {
                                    // Surragete pair encoding: 0x10000 + (Hi - 0xD800) * 0x400 + (Lo - 0xDC00)
                                    code = (static_cast<unsigned>(code) << 10U) + static_cast<unsigned>(low) -
                                           ((0xd800 << 10U) + 0xdc00U - 0x10000U);
                                }
                            }
                        }
                    }
