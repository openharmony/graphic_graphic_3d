...
/**
 * @tc.name: coreLog
 * @tc.desc: Tests for Core Log. [AUTO-GENERATED]
 * @tc.type: FUNC
 */
UNIT_TEST(SRC_LoggerTest, coreLog, testing::ext::TestSize.Level1)
{
    CORE_LOG_V("Verbose level log message");
    CORE_LOG_D("Debug level log message");
    CORE_LOG_I("Info level log message");
    CORE_LOG_W("Warning level log message");
    CORE_LOG_E("Error level log message");
    CORE_LOG_F("Fatal level log message");

    // Check that parsing both / and \ work
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "path/file.x", 0, "File path with '/' 1");
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "path\\file.x", 0, "File path with '\\' 1");
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "/file.x", 0, "File path with '/' 2");
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "\\file.x", 0, "File path with '\\' 2");
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "/", 0, "File path with '/' 3");
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "\\", 0, "File path with '\\' 3");

    GetLogger().Log(ILogger::LogLevel::LOG_INFO, "", 123, "File path empty");
    GetLogger().Log(ILogger::LogLevel::LOG_INFO, nullptr, 123, "File path null");
    GetLogger().Log(ILogger::LogLevel::LOG_ERROR, nullptr, 123, "File path null");
}

/**
 * @tc.name: coreLogUtf8
 * @tc.desc: Tests for Core Log Utf8. [AUTO-GENERATED]
 * @tc.type: FUNC
 */
UNIT_TEST(SRC_LoggerTest, coreLogUtf8, testing::ext::TestSize.Level1)
{
    CORE_LOG_I("äää ööö");
    CORE_LOG_I("文本");
    CORE_LOG_I("نص");
...
/**
 * @tc.name: coreLogOnce
 * @tc.desc: Tests for Core Log Once. [AUTO-GENERATED]
 * @tc.type: FUNC
 */
UNIT_TEST(SRC_LoggerTest, coreLogOnce, testing::ext::TestSize.Level1)
{
    for (int i = 0; i < 20; i++)
    {
        CORE_ONCE("logger1", CORE_LOG_E("Log this only once (%d)", i));
    }
    for (int i = 0; i < 20; i++)
    {
        CORE_ONCE("logger2", CORE_LOG_E("Different log once %d", i));
    }

    CORE_ONCE("logger1", CORE_LOG_E("Log this only once"));


    CORE_ONCE_RESET();
    for (int i = 0; i < 20; i++)
    {
        CORE_ONCE("logger1", CORE_LOG_V("Log this only once2 (%d)", i));
    }
}
