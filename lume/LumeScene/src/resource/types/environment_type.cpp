...
bool EnvironmentResourceType::Build(const META_NS::IMetadata::Ptr& d)
{
    bool res = Super::Build(d);
    if (res) {
        auto context = GetInterfaceBuildArg<IRenderContext>(d, "RenderContext");
        if (!context) {
            CORE_LOG_E("Invalid arguments to construct EnvironmentResourceType");
            return false;
        }
        context_ = context;
    }
...
BASE_NS::string EnvironmentResourceType::GetResourceName() const
{
    return "Environment";
}
BASE_NS::Uid EnvironmentResourceType::GetResourceType() const
{
...
CORE_NS::IResource::Ptr EnvironmentResourceType::LoadResource(const StorageInfo& s) const
{
    ...
    CORE_NS::IResource::Ptr res = scene->CreateObject<CORE_NS::IResource>(ClassId::Environment).GetResult();
    if (res && s.options) {
        if (auto opts = META_NS::GetObjectRegistry().Create<META_NS::IObjectResourceOptions>(
            ...
            if (auto i = interface_cast<META_NS::IDerivedFromResource>(res)) {
                ...
                if (base.IsValid()) {
                    ...
                    if (!r) {
                        CORE_LOG_W("Could not load base resource for environment %s", s.id.ToString().c_str());
                    }
                    i->SetResource(r);
                }
            }
