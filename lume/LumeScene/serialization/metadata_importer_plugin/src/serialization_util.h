...
#ifndef SCENE_MIMP_SRC_SERIALIZATION_UTIL_H
#define SCENE_MIMP_SRC_SERIALIZATION_UTIL_H
...
void AddFlatProperties(const META_NS::IMetadata& in, META_NS::IObject& parent, bool isTemplateContext);
...
inline void SerCopy(const META_NS::IMetadata& in, META_NS::IMetadata& out, bool isTemplateContext = false)
{
    if (auto m = interface_cast<META_NS::IObject>(&out)) {
        AddFlatProperties(in, *m, isTemplateContext);
    }
}
...
bool SerialiseAttachment(const Obj& obj)
{
    return META_NS::IsFlagSet(obj, META_NS::ObjectFlagBits::SERIALIZE) && !interface_cast<META_NS::IEvent>(obj) &&
           ...
           !interface_cast<IComponent>(obj) && !interface_cast<IExternalNode>(obj);
}
