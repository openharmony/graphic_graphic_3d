...
{
    if (s.options) {
        CreateObjectResourceOptions(p, s.self, s.context, *s.options);
    }
    return true;
}
bool MaterialResourceType::ReloadResource(const StorageInfo& s, const CORE_NS::IResource::Ptr& res) const
{
    if (res && s.options) {
        ApplyObjectResourceOptions(s.id, res, *s.options, s.self, s.context);
        return true;
...
bool OcclusionMaterialResourceType::ReloadResource(const StorageInfo& s, const CORE_NS::IResource::Ptr& res) const
{
    if (res && s.options) {
        ApplyObjectResourceOptions(s.id, res, *s.options, s.self, s.context);
        return true;
