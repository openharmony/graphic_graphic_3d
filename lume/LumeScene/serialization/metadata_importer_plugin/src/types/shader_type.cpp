...
CORE_NS::IResource::Ptr ShaderResourceType::LoadResource(const StorageInfo& s) const
{
    ...
    if (res && s.options) {
        ApplyObjectResourceOptions(s.id, res, *s.options, nullptr, s.context);
    }
    return res;
}
...
{
    if (s.options) {
        CreateObjectResourceOptions(p, nullptr, s.context, *s.options);
    }
    return true;
}
bool ShaderResourceType::ReloadResource(const StorageInfo& s, const CORE_NS::IResource::Ptr& p) const
{
    ...
    if (p && s.options && !s.payload) {
        ApplyObjectResourceOptions(s.id, p, *s.options, nullptr, s.context);
        return true;
    }
