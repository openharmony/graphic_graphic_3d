...
CORE_NS::IResource::Ptr AnimationResourceType::LoadResource(const StorageInfo& s) const
{
    ...
    CORE_NS::IResource::Ptr res = is->RunDirectlyOrInTask(
        [&]() { return interface_pointer_cast<CORE_NS::IResource>(is->ConstructAnimation(s.id)); });
    if (res && s.options) {
...
bool AnimationResourceType::ReloadResource(const StorageInfo& s, const CORE_NS::IResource::Ptr& res) const
{
    if (res && s.options) {
        ApplyObjectResourceOptions(s.id, res, *s.options, s.self, s.context);
        return true;
