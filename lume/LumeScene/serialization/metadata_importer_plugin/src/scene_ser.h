...
#ifndef SCENE_SRC_SCENE_SER_H
#define SCENE_SRC_SCENE_SER_H
...
class IComponentSer : public CORE_NS::IInterface {
    META_INTERFACE(CORE_NS::IInterface, IComponentSer, "845e5ead-ceca-45a2-a180-1841dd45755f")
public:
    ...
    virtual void SetId(const META_NS::ObjectId& id) = 0;
};

SCENE_END_NAMESPACE()
META_INTERFACE_TYPE(SCENE_NS::IComponentSer)
SCENE_BEGIN_NAMESPACE()
...
public:
    ...
    virtual void SetComponents(BASE_NS::vector<IComponentSer::Ptr>) = 0;
    virtual BASE_NS::vector<IComponentSer::Ptr> GetComponents() const = 0;
};
...
public:
    ...
    virtual void AddComponent(BASE_NS::string_view path, IComponentSer::Ptr) = 0;
    virtual BASE_NS::vector<IExternalAttachment::Ptr> GetComponents() const = 0;
};
...
META_REGISTER_CLASS(ComponentSer, "2ed8a93e-c860-46a4-aae4-83b7084b1cfb", META_NS::ObjectCategoryBits::NO_CATEGORY)
...
public:
    ...
    void SetComponents(BASE_NS::vector<IComponentSer::Ptr> comps) override
    {
        components_ = BASE_NS::move(comps);
    }
    BASE_NS::vector<IComponentSer::Ptr> GetComponents() const override
    {
        return components_;
    }
    ...
    META_NS::ReturnError Export(META_NS::IExportContext& c) const override
    {
        ...
        return META_NS::Serializer(c) & META_NS::AutoSerialize() & META_NS::NamedValue("id", id_) &
               META_NS::NamedValue("attachments", attas_) & META_NS::NamedValue("components", components_);
    }
    META_NS::ReturnError Import(META_NS::IImportContext& c) override
    {
        ...
        if (res & META_NS::AutoSerialize() & META_NS::NamedValue("id", id_) &
            ...
            name_ = c.GetName();
            if (c.HasMember("components")) {
                res& META_NS::NamedValue("components", components_);
            }
        }
        return res;
    }
...
private:
    ...
    BASE_NS::vector<IComponentSer::Ptr> components_;
};
...
public:
    ...
    void AddComponent(BASE_NS::string_view path, IComponentSer::Ptr obj) override
    {
        auto ea = META_NS::GetObjectRegistry().Create<IExternalAttachment>(ClassId::ExternalAttachment);
        if (ea) {
            ea->SetPath(path);
            ea->SetAttachment(interface_pointer_cast<META_NS::IObject>(obj));
            components_.push_back(ea);
        }
    }
    BASE_NS::vector<IExternalAttachment::Ptr> GetComponents() const override
    {
        return components_;
    }
    ...
    META_NS::ReturnError Export(META_NS::IExportContext& c) const override
    {
        ...
        return META_NS::Serializer(c) & META_NS::AutoSerialize() & META_NS::NamedValue("resourceId.name", id_.name) &
               ...
               META_NS::NamedValue("nodeComponents", components_);
    }
    META_NS::ReturnError Import(META_NS::IImportContext& c) override
    {
        META_NS::Serializer res(c);
        if (res & META_NS::AutoSerialize() & META_NS::NamedValue("resourceId.name", id_.name) &
            ...
            if (c.HasMember("nodeComponents")) {
                res& META_NS::NamedValue("nodeComponents", components_);
            }
        }
        return res;
    }
...
private:
    ...
    BASE_NS::vector<IExternalAttachment::Ptr> components_;
};
...
class ComponentSer : public META_NS::IntroduceInterfaces<META_NS::MetaObject, IComponentSer, META_NS::ISerializable> {
    META_OBJECT(ComponentSer, ClassId::ComponentSer, IntroduceInterfaces)
public:
    BASE_NS::string GetName() const override
    {
        return name_;
    ...
    void SetId(const META_NS::ObjectId& id) override
    {
        id_ = id;
    }
    ...
    {
        // the name is automatically serialised for objects using the GetName in IObject
        return META_NS::Serializer(c) & META_NS::AutoSerialize() & META_NS::NamedValue("componentId", id_);
    }
    META_NS::ReturnError Import(META_NS::IImportContext& c) override
    {
        META_NS::Serializer res(c);
        if (res & META_NS::AutoSerialize() & META_NS::NamedValue("componentId", id_)) {
            name_ = c.GetName();
        }
        return res;
    }
