...
public:
    virtual void SetValueBase(const std::any &value) = 0;
    virtual std::any GetValueBase() const = 0;
...
public:
    ...
    void SetValueBase(const std::any &value) override
    {
        if (const Type *vp = std::any_cast<Type>(&value)) {
            META_NS::Property<Type>(prop_)->SetValue(*vp);
        } else {
            CORE_LOG_E("set property value failed, type  mismatch. recommend to use SetValue instead.");
        }
    }

    std::any GetValueBase() const override
    {
