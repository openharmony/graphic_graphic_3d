    ...
    const uint32_t width, const uint32_t height)
{
    ...
    imageDesc.imageViewType = IImageContainer::ImageViewType::VIEW_TYPE_2D;
    imageDesc.blockPixelDepth = 1;
    imageDesc.imageType = IImageContainer::ImageType::TYPE_2D;
    ...
    gpuImageDesc.engineCreationFlags = EngineImageCreationFlagBits::CORE_ENGINE_IMAGE_CREATION_DYNAMIC_BARRIERS;
...
RenderHandleReference CreateTexture(shared_ptr<IRenderContext> rc, const Base::string_view& name, const Format format,
    const uint32_t width, const uint32_t height)
{
    IGpuResourceManager& gpuResourceMgr = rc->GetDevice().GetGpuResourceManager();
    auto gpuImageDesc = GetGpuImageDesc(rc, name, format, width, height);
    ImageUsageFlags usageFlags = CORE_IMAGE_USAGE_SAMPLED_BIT | CORE_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
                                 CORE_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | CORE_IMAGE_USAGE_TRANSFER_SRC_BIT;
    gpuImageDesc.usageFlags = usageFlags;
    auto imageHandle = gpuResourceMgr.Create(name, gpuImageDesc);
    return imageHandle;
}

RenderHandleReference CreateTextureDepth(shared_ptr<IRenderContext> rc, const Base::string_view& name,
...
RenderHandleReference CreateRenderNodeGraphFG(shared_ptr<IRenderContext> renderContext_, const string_view rngPath)
{
    IRenderNodeGraphManager& graphManager = renderContext_->GetRenderNodeGraphManager();
    const RenderHandleReference handle = graphManager.LoadAndCreate(
...
void FGModule::Init(shared_ptr<SCENE_NS::IInternalScene> scene, shared_ptr<IRenderContext> renderContext,
    shared_ptr<CORE_NS::IEngine> engine, refcnt_ptr<CORE_NS::IEcs> ecs)
{
    ecs_ = ecs;
    rngPredict_ = CreateRenderNodeGraphFG(renderContext, "fg_rofs://rendernodegraphs/fg_predict.rng");
    const string_view display_real = fg_.withSR_ ? "fg_rofs://rendernodegraphs/fg_display_real_with_sr.rng"
                                                 : "fg_rofs://rendernodegraphs/fg_display_real.rng";
    rngDisplayReal_ = CreateRenderNodeGraphFG(renderContext, display_real);
    const string_view displayPredict = fg_.withSR_ ? "fg_rofs://rendernodegraphs/fg_display_predict_with_sr.rng"
...
void FGModule::CreateGpuImages(shared_ptr<IRenderContext> rc, float width, float height, const FGData& fg,
